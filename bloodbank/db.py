...
def connect():
    if is_pg():
        # Prefer psycopg v3. It supports newer Python runtimes better than psycopg2
        # and returns dict-like rows via dict_row. Fallback to psycopg2 for older installs.
        try:
            import psycopg
...
def init_db():
    with db() as conn:
        ...
        default_admin_username = current_app.config.get("DEFAULT_ADMIN_USERNAME", "Sepsis")
        default_admin_password = current_app.config.get("DEFAULT_ADMIN_PASSWORD", "Sepsis1986")
        exists = one(conn, f"SELECT id FROM users WHERE username={p}", (default_admin_username,))
        any_admin = one(conn, "SELECT id FROM users WHERE role='admin' AND active=1 LIMIT 1")
        if not exists and not any_admin:
            sql = "INSERT INTO users(username,full_name,position,role,password_hash,active,first_login) VALUES (?,?,?,?,?,1,0)"
            if is_pg():
                sql = "INSERT INTO users(username,full_name,position,role,password_hash,active,first_login) VALUES (%s,%s,%s,%s,%s,1,0)"
            exec_sql(conn, sql, (default_admin_username, "Адміністратор", "Завідувач", "admin", generate_password_hash(default_admin_password)))
