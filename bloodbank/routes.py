...
@bp.route("/api/emergency/reset-admin", methods=["GET", "POST"])
def emergency_reset_admin():
    """Emergency admin recovery. Requires API_TOKEN in Render env."""
    ...
    if not expected or not token or token != expected:
        return err("Недійсний або відсутній API_TOKEN", 403)

    username = clean_text(request.args.get("username") or current_app.config.get("DEFAULT_ADMIN_USERNAME") or "Sepsis", 120)
    password = request.args.get("password") or current_app.config.get("DEFAULT_ADMIN_PASSWORD") or "Sepsis1986"
    if len(password) < 6:
        return err("Пароль має бути мінімум 6 символів", 400)

    p = ph()
    with db() as conn:
        u = one(conn, f"SELECT id FROM users WHERE username={p}", (username,))
        password_hash = generate_password_hash(password)
        if u:
            exec_sql(conn, f"""UPDATE users
                SET full_name=COALESCE(NULLIF(full_name,''),'Адміністратор'),
                    position=COALESCE(NULLIF(position,''),'Завідувач'),
                    role='admin', password_hash={p}, active=1, first_login=0
                WHERE username={p}""", (password_hash, username))
            action = "updated"
            user_id = u["id"]
        else:
            sql = f"INSERT INTO users(username,full_name,position,role,password_hash,active,first_login) VALUES ({p},{p},{p},'admin',{p},1,0)"
            cur = exec_sql(conn, sql, (username, "Адміністратор", "Завідувач", password_hash))
            user_id = getattr(cur, "lastrowid", None)
            action = "created"
        try:
            audit(conn, "emergency_reset_admin", "users", user_id)
        except Exception:
            pass
    return ok(action=action, username=username, password=password, message="Адміністратора відновлено")
