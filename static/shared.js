...
// V6.4.40: звіт за замовчуванням відкривається на поточний місяць, а не на старий тестовий місяць.
function initMozReportPeriodV6436(){
  const d=new Date();
  const y=document.getElementById('moz_year'); if(y && (!y.value || y.value==='2026')) y.value=String(d.getFullYear());
  const m=document.getElementById('moz_month'); if(m) m.value=String(d.getMonth()+1);
  const q=document.getElementById('moz_quarter'); if(q) q.value=String(Math.floor(d.getMonth()/3)+1);
}
document.addEventListener('DOMContentLoaded',()=>setTimeout(initMozReportPeriodV6436,50));
...
async function loadStock(){
  ...
  let st=[]; try{st=await jget('/api/stock',{ttl:0,timeout:30000});}catch(e){st=[]}
  let entries=[]; try{let r=await jget('/api/stock/entries',{ttl:0,timeout:30000}); entries=r.items||[]}catch(e){entries=[]}
  ...
  st.forEach(x=>{let q=Number(x.qty||0);if(compKind(x.component)=='rbc')r+=q;if(compKind(x.component)=='plasma')p+=q;if(compKind(x.component)=='plt')t+=q;if(compKind(x.component)=='cryo')cr+=q;if(q<5)low++;summary+=`<tr><td>${escHtml(x.component)}</td><td>${escHtml(x.donor_group||x.group||'')}</td><td>${escHtml(x.donor_rh||x.rh||'')}</td><td>${q}</td></tr>`});
  summary+='</table></div>';
  ...
  const isIncoming=x=>String(x.type||'').toLowerCase().includes('надход') || String(x.type||'').toLowerCase().includes('in') || String(x.type||'').toLowerCase().includes('прихід');
  const incoming=entries.filter(isIncoming);
  const outgoing=entries.filter(x=>!isIncoming(x));
  const renderCard=(x, positive)=>`<div class="stock-entry-card ${positive?'stock-in-card':'stock-out-card'}">
        ...
        <div class="stock-entry-grid">
          ...
          ${x.patient_name?`<div><b>Пацієнт</b><span>${escHtml(x.patient_name)}</span></div>`:''}
          ${x.note?`<div class="stock-note"><b>Опис</b><span>${escHtml(x.note)}</span></div>`:''}
        </div>
  ...
  let list='<h3>Рух складу</h3><div class="notice">Надходження і списання/видача показані окремими стовпчиками, щоб не плутати плюс і мінус.</div>';
  list+=`<div class="stock-movement-columns">
    <div class="stock-movement-col"><h3>⬇️ Надходження <span>${incoming.length}</span></h3>${incoming.length?incoming.map(x=>renderCard(x,true)).join(''):'<div class="notice">Надходжень немає</div>'}</div>
    <div class="stock-movement-col"><h3>⬆️ Списання / видача <span>${outgoing.length}</span></h3>${outgoing.length?outgoing.map(x=>renderCard(x,false)).join(''):'<div class="notice">Списань або видач немає</div>'}</div>
  </div>`;
...
function requestFullDetailsHtml(x){
  ...
  const rows=[
    ['№ вимоги', x.id], ['Дата створення', x.created_at], ['Потрібно на дату', x.needed_date],
    ['ПІБ пацієнта', x.patient_name], ['Дата народження', x.birth_date], ['Статус пацієнта', x.patient_status],
    ['Адреса / проживання', x.address], ['Відділення', x.department],
    ['Компонент', x.component], ['Група/Rh пацієнта', `${x.patient_group||''} ${x.patient_rh||''}`.trim()],
    ['Кількість', x.amount], ['Терміновість', x.urgency], ['Діагноз', x.diagnosis], ['Примітка', x.note],
    ['Лікар, що створив', x.doctor_name], ['Посада лікаря', x.doctor_position], ['ID створив', x.created_by],
    ['Статус вимоги', x.status], ['Погодив', x.approved_by], ['Резервував', x.reserved_by], ['Дата резерву', x.reserved_at],
    ['Донорська група/Rh', `${x.donor_group||''} ${x.donor_rh||''}`.trim()], ['Пакет', x.pack_no], ['Серія', x.series], ['Термін компонента', x.expiry], ['unit_id', x.unit_id],
    ['Видав', x.issued_by], ['Дата видачі', x.issued_at], ['Використано', x.used_at], ['Підтвердив використання', x.used_by],
    ['Списано', x.writeoff_at], ['Хто списав', x.written_by], ['Причина списання', x.writeoff_reason],
    ['Реакція', x.reaction_present], ['Тип реакції', x.reaction_type], ['Тяжкість реакції', x.reaction_severity], ['Опис реакції', x.reaction_description], ['Наслідок реакції', x.reaction_result],
    ['Сумісність', x.compatibility_ok], ['Попередження сумісності', x.compatibility_warning]
  ].filter(([_,v])=>v!==undefined && v!==null && String(v).trim()!=='');
  return `<div class="req-full-details"><b>Повні дані вимоги</b><div class="req-full-grid">${rows.map(([k,v])=>`<div><b>${escHtml(k)}</b><span>${escHtml(v)}</span></div>`).join('')}</div></div>`;
}
...
async function loadRequestsLive(){
  ...
  filtered.forEach(x=>{
    h+=`<div class="req-card ${x.status=='Нова'?'live-new':''}">
      ...
      ${x.needed_date?`<div class="notice mini-notice"><b>Потрібно на дату:</b> ${escHtml(x.needed_date)}</div>`:''}
      ${x.compatibility_warning?`<div class="danger">${x.compatibility_warning}</div>`:''}
      ${requestFullDetailsHtml(x)}
...
function canWriteoffRequestUI(x, tab='active'){
  ...
  // V6.4.70: списання вимоги є тільки для вже виданого компонента.
  // Для Погоджено/Зарезервовано треба використовувати Відмовити або Скасувати видачу/резерв,
  // інакше UI показує кнопку, яку backend законно блокує.
  if(['used','written','rejected'].includes(tab)) return false;
  if(st!=='Видано') return false;
  return ['admin','transfusion','doctor','nurse'].includes(role);
}
function actions(x, tab='active'){
  ...
  if(!archived && ['admin','transfusion'].includes(role)){
    h+=`<button class="btn-blue" onclick="reqAction(${x.id},'approve')">Погодити</button><button class="btn-orange" onclick="reqAction(${x.id},'reserve')">Резерв</button><button class="btn-orange" onclick="reqAction(${x.id},'issue')">Видати</button><button class="btn-red" onclick="reqAction(${x.id},'reject')">Відмовити</button>`;
    if(String(x.status||'').trim()==='Видано'){
      h+=`<button class="btn-red" onclick="cancelIssue(${x.id})">Скасувати видачу</button>`;
    }
  }
  if(!archived){
    if(['admin','transfusion'].includes(role) && String(x.status||'').trim()==='Видано'){
      h+=`<button class="btn-green" onclick="openUsedModal(${x.id})">Використано</button>`;
    }
    if(canWriteoffRequestUI(x, tab)){
      h+=`<button class="btn-red req-writeoff-btn" onclick="openWriteoffModal(${x.id})">Списати виданий компонент</button>`;
    }
...
async function createRequest(){
  if(!requireFields(['patient_name','birth_date','patient_status','department','component','patient_group','patient_rh','amount','urgency','diagnosis']))return;let d={patient_name:val('patient_name'),birth_date:val('birth_date'),address:val('address'),patient_status:val('patient_status'),department:val('department'),component:val('component'),patient_group:val('patient_group'),patient_rh:val('patient_rh'),amount:val('amount'),urgency:val('urgency'),needed_date:val('needed_date'),diagnosis:val('diagnosis'),note:val('note')};let j=await jpost('/api/request/create',d);toast(j.ok?'✅ Вимогу створено':(j.error||'Помилка'),'good');;loadAll();}
async function reqAction(id,action){let d={id,action};if(action=='issue'||action=='reserve'){d.donor_group=prompt('Група донора','');d.donor_rh=prompt('Rh','');d.pack_no=prompt('№ пакета','');d.series=prompt('Серія','');d.expiry=prompt('Термін','');if(action=='reserve'){let nd=prompt('Дата, на яку потрібен компонент (YYYY-MM-DD). Можна залишити порожнім','');if(nd!==null)d.needed_date=nd;}if(action=='issue')d.override=confirm('Дозволити видачу при попередженні несумісності?')}let j=await jpost('/api/request/action',d);alert(j.ok?'Готово':j.error);loadAll();}
...
async function cancelIssue(id){let reason=prompt('Причина скасування видачі','Помилкова видача / скасовано');if(!reason)return;let j=await jpost('/api/request/cancel-issue',{id,reason});alert(j.ok?(j.message||'Видачу скасовано'):j.error);loadAll();}
function openWriteoffModal(id){
  ...
  jpost('/api/request/writeoff',{id,writeoff_date,written_by,writeoff_reason}).then(j=>{alert(j.ok?'Списано виданий компонент':j.error);loadAll();});
}
...
async function editRequest(id){
  ...
  const locked=['Погоджено','Зарезервовано','Резерв','Видано','Використано','Списано','Відмовлено','Потребує перегляду'].includes(st);
  let d={id};
  ...
  d.patient_status=prompt('Статус пацієнта',x.patient_status||''); if(d.patient_status===null)return;
  d.address=prompt('Адреса / проживання хворого',x.address||''); if(d.address===null)return;
  ...
  if(!locked){
    ...
    d.amount=prompt('Кількість',x.amount||''); if(d.amount===null)return;
    d.needed_date=prompt('Дата, на яку потрібен компонент YYYY-MM-DD',x.needed_date||''); if(d.needed_date===null)return;
  } else {
    toast('Оброблена вимога: критичні поля (компонент, група, Rh, кількість, дата потреби, статус) змінюються тільки контрольованими діями','warn');
  }
  // V6.4.70: статус не редагується вручну через форму.
...
async function loadReport(){
  ...
  h+='<div class="report-cards">'+rows.map(x=>{
    ...
    return `<div class="${cls} report-edit-card" data-component="${escHtml(x.component)}"><h4>${escHtml(x.component)}</h4><div class="report-metric-grid">
      <label><span>Залишок поч.</span><input class="report-edit" data-metric="opening_total" type="number" step="0.01" value="${fmt(x.opening_total)}"></label>
      <label><span>Одержано</span><input class="report-edit" data-metric="received_total" type="number" step="0.01" value="${fmt(x.received_total)}"></label>
      <label><span>Використано/видано</span><input class="report-edit" data-metric="used_total" type="number" step="0.01" value="${fmt(x.used_total)}"></label>
      <label><span>Залишок кін.</span><input class="report-edit" data-metric="closing_total" type="number" step="0.01" value="${fmt(x.closing_total)}"></label>
      <label><span>Списано строк</span><input class="report-edit" data-metric="expired_total" type="number" step="0.01" value="${fmt(x.expired_total)}"></label>
      <label><span>Списано інше</span><input class="report-edit" data-metric="other_writeoff_total" type="number" step="0.01" value="${fmt(x.other_writeoff_total)}"></label>
    </div></div>`;
  }).join('')+'</div>';
...
async function saveReportEdits(){
  const cards=[...document.querySelectorAll('#reportPreview .report-edit-card')];
  if(!cards.length){alert('Спочатку натисни “Попередній перегляд”.');return;}
  const rows=cards.map(c=>{
    const o={component:c.dataset.component||''};
    c.querySelectorAll('input.report-edit').forEach(i=>{o[i.dataset.metric]=i.value||0});
    return o;
  });
  const note=prompt('Примітка до ручної правки звіту','Ручна правка звіту');
  if(note===null)return;
  const j=await jpost('/api/reports/moz-period-overrides?'+mozReportQuery(),{rows,note});
  alert(j.ok?'Правки звіту збережено':(j.error||'Помилка'));
  if(j.ok)loadReport();
}
async function clearReportEdits(){
  if(!confirm('Скасувати ручні правки звіту за вибраний період і знову рахувати з історії складу?'))return;
  const j=await jpost('/api/reports/moz-period-overrides/clear?'+mozReportQuery(),{});
  alert(j.ok?'Правки скасовано':(j.error||'Помилка'));
  if(j.ok)loadReport();
}
async function clearHistoryForReportPeriod(){
  const reason=prompt('Причина очищення історії за вибраний період','Помилковий/тестовий період');
  if(reason===null)return;
  if(!confirm('Історія за вибраний період буде прихована зі звітів. Дані не видаляються фізично і можуть бути відновлені. Продовжити?'))return;
  const j=await jpost('/api/history/clear-period?'+mozReportQuery(),{reason});
  alert(j.ok?j.message:(j.error||'Помилка'));
  loadHistoryPeriodActions(); loadReport();
}
async function loadHistoryPeriodActions(){
  const box=document.getElementById('historyPeriodActions'); if(!box)return;
  const d=await jget('/api/history/period-actions',{ttl:0});
  const arr=d.rows||[];
  if(!arr.length){box.innerHTML='<div class="muted">Очищень історії ще немає.</div>';return;}
  box.innerHTML='<div class="table-scroll"><table><tr><th>ID</th><th>Створено</th><th>Період</th><th>Причина</th><th>Статус</th><th>Дія</th></tr>'+arr.map(x=>`<tr><td>${escHtml(x.id)}</td><td>${escHtml(x.created_at||'')}</td><td>${escHtml(x.period_start||'')} — ${escHtml(x.period_end||'')}</td><td>${escHtml(x.reason||'')}</td><td>${Number(x.active)?'Очищено':'Відновлено'}</td><td>${Number(x.active)?`<button onclick="restoreHistoryPeriod(${x.id})">Відновити</button>`:''}</td></tr>`).join('')+'</table></div>';
}
async function restoreHistoryPeriod(id){
  if(!confirm('Відновити історію цього періоду у звітах?'))return;
  const j=await jpost('/api/history/restore-period',{id});
  alert(j.ok?j.message:(j.error||'Помилка'));
  loadHistoryPeriodActions(); loadReport();
}
...
async function loadHomeStatusV6424(){
  try{ const h=await jget('/api/health'); updateHomeSystemStatusV6424(h); }catch(e){
    updateHomeSystemStatusV6424({ok:false,version:'V6.4.70',database_status_label:'❌ Недоступно',database_type:'—',backup_status_label:'—',telegram_status_label:'—'});
  }
}
...
const ROLE_SECTION_MAP={
  admin:["dashboard","stock","requests","reports","users","telegram","telegramPersonal","pwa","monitor","audit","maintenance","barcode","traceability","incompat","dashboardPro","temperature","writeoff","trash","backup","trash"],
  transfusion:["dashboard","stock","requests","reports","users","telegram","telegramPersonal","pwa","monitor","audit","maintenance","barcode","traceability","incompat","dashboardPro","temperature","writeoff","trash"],
...
(function(){
  const BB_FEATURE_ALIASES_V619 = {
    ...
    telegram:'telegramPersonalSec', telegramPersonal:'telegramPersonalSec',
    pwa:'pwaInstallSec', reports:'reportsSec', users:'usersSec', audit:'auditSec', backup:'backupSec',
    traceability:'traceabilitySec', incompat:'incompatSec', writeoff:'writeoffSec', dailyReport:'reportsSec',
    components:'stockSec', reactions:'reactionRegistrySec', warnings:'warningsSec', warningSec:'warningsSec',
...
function updateNavigationActiveV647(id){
  try{
    ...
    const titleMap={homeSec:'🏠 Головна',requestsSec:'Вимоги',stockSec:'Компоненти і склад',alertsSec:'Попередження',patientsSec:'Пацієнт',transfusionJournalSec:'Журнал трансфузій',qrScannerSec:'QR',reactionRegistrySec:'Реакції',signSec:'Підпис',telegramSec:'Налаштування Telegram-бота',usersSec:'Користувачі',auditSec:'Журнал',reportsSec:'Звіти',backupSec:'Резервні копії',trashSec:'Кошик',maintenanceSec:'Моніторинг',pwaInstallSec:'Додаток',telegramPersonalSec:'Мої Telegram-сповіщення',barcodeSec:'QR/Barcode',traceabilitySec:'Простежуваність',incompatSec:'Несумісності',dashboardProSec:'Розширена панель',temperatureSec:'Температура',writeoffSec:'Списання',dailyReportSec:'Звіти',componentsSec:'Компоненти і склад',warningsSec:'Попередження складу'};
...
async function loadSectionDataV642(id){
  ...
  try{
    ...
    else if(id==='stockSec'){ tasks.push(runIfV642('loadStock')); tasks.push(runIfV642('loadComponentStockV613')); }
    ...
    else if(id==='telegramPersonalSec') {
      tasks.push(runIfV642('loadTelegramMe'));
      if(['admin','transfusion'].includes(ROLE)){
        tasks.push(runIfV642('loadTelegramStatus'), runIfV642('loadTelegramLogs'), runIfV642('loadTelegramQueue'));
      }
    }
    ...
    else if(id==='dailyReportSec') { show('reportsSec'); return; }
    else if(id==='componentsSec') { show('stockSec'); return; }
...
async function applyRoleVisibilityV613(){
  ...
  const allowed={
    admin:['homeSec','stockSec','requestsSec','patientsSec','warningsSec','traceabilitySec','temperatureSec','dashboardProSec','barcodeSec','transfusionJournalSec','usersSec','auditSec','trashSec','backupSec','settingsSec','maintenanceSec','telegramPersonalSec','pwaInstallSec','reportsSec','reactionRegistrySec','signSec'],
    transfusion:['homeSec','stockSec','requestsSec','patientsSec','warningsSec','traceabilitySec','temperatureSec','dashboardProSec','barcodeSec','transfusionJournalSec','settingsSec','maintenanceSec','telegramPersonalSec','pwaInstallSec','reportsSec','usersSec','auditSec','trashSec','reactionRegistrySec','signSec'],
...
function requestFullDescriptionV640(r){
  const pairs=[
    ['№ вимоги', r.id], ['Дата створення', r.created_at], ['ПІБ пацієнта', r.patient_name], ['Дата народження', r.birth_date],
    ['Статус пацієнта', r.patient_status], ['Адреса', r.address], ['Відділення', r.department], ['Компонент у вимозі', r.component],
    ['Група пацієнта', r.patient_group], ['Rh пацієнта', r.patient_rh], ['Кількість', r.amount], ['Терміновість', r.urgency],
    ['Діагноз', r.diagnosis], ['Примітка', r.note], ['Статус вимоги', r.status], ['Створив ID', r.created_by],
    ['Лікар', r.doctor_name], ['Посада лікаря', r.doctor_position], ['Сумісність OK', r.compatibility_ok], ['Попередження сумісності', r.compatibility_warning],
    ['Погодив', r.approved_by], ['Видав', r.issued_by], ['Дата видачі', r.issued_at], ['Пакет', r.pack_no], ['Серія', r.series], ['Термін придатності', r.expiry]
  ];
  return '<div class="component-request-description">'+pairs.filter(p=>p[1]!==undefined && p[1]!==null && String(p[1]).trim()!=='').map(p=>`<div><b>${escapeHtmlV630(p[0])}</b><span>${escapeHtmlV630(p[1])}</span></div>`).join('')+'</div>';
}
async function componentRelatedRequestsV640(component, donor_group, donor_rh){
  try{
    const qs=new URLSearchParams({component:component||'',donor_group:donor_group||'',donor_rh:donor_rh||''});
    const r=await jget('/api/component-stock/approved-requests?'+qs.toString(),{ttl:0,timeout:30000});
    return (r&&r.items)||[];
  }catch(e){return []}
}
...
async function loadComponentStockV613(force=false){
  ...
  const items=(res.items||[]).map(x=>({
    ...
    available_qty:Number(x.available_qty ?? (x.qty ?? x.amount ?? x.total ?? 0)),
    packs:x.packs||'',
    lot_count:Number(x.lot_count ?? x.packs ?? 0),
    movement_count:Number(x.movement_count ?? 0),
  ...
  for(const comp of comps){
    ...
    const rowBits=[];
    for(const x of rows){
      const idx=window.COMPONENT_STOCK_GROUPS.push({component:comp, donor_group:x.donor_group, donor_rh:x.donor_rh, qty:x.available_qty, total_qty:x.qty, reserved_qty:x.reserved_qty, expiry:x.nearest_expiry||'', pack_no:x.pack_no||'', series:x.series||''})-1;
      const reqs=await componentRelatedRequestsV640(comp, x.donor_group, x.donor_rh);
      const reqHtml=reqs.length?`<details class="component-linked-requests"><summary>Пов’язані погоджені вимоги: ${reqs.length}</summary>${reqs.slice(0,8).map(requestFullDescriptionV640).join('')}</details>`:'<div class="component-linked-empty">Пов’язаних погоджених вимог немає</div>';
      rowBits.push(`<div class="component-row">
        <div><b>Група</b><span>${escapeHtmlV630(x.donor_group)}</span></div>
        <div><b>Rh</b><span>${escapeHtmlV630(x.donor_rh)}</span></div>
        <div><b>Залишок</b><span>${escapeHtmlV630(String(x.qty))}</span></div>
        <div><b>Доступно</b><span>${escapeHtmlV630(String(x.available_qty))}</span></div>
        <div><b>Резерв</b><span>${escapeHtmlV630(String(x.reserved_qty||0))}</span></div>
        <div><b>Партій</b><span>${escapeHtmlV630(String(x.lot_count||x.packs||0))}</span></div>
        <div><b>Рухів</b><span>${escapeHtmlV630(String(x.movement_count||0))}</span></div>
        ...
        <div class="component-row-description">${reqHtml}</div>
      </div>`);
    }
    const rowHtml=rowBits.join('');
    htmlParts.push(`<details class="component-detail ${componentColorV613(comp)}">
...
async function editComponentStockV6417(idx){
  ...
  alert('Безпечний режим: назву компонента, групу, Rh і термін можна змінювати тільки в конкретному записі журналу складу. Тут доступна лише корекція кількості конкретної партії.');
  const qty=prompt('Нова загальна кількість для цієї позиції', String(x.qty??'')); if(qty===null)return;
  const pack_no=prompt('Пакет партії для корекції', x.pack_no||'') || '';
  const series=prompt('Серія партії для корекції', x.series||'') || '';
  if(!String(pack_no).trim() && !String(series).trim()){toast('Потрібно вказати пакет або серію партії','warn');return;}
  const r=await jpost('/api/component-stock/update',{
    ...
    component:x.component, donor_group:x.donor_group, donor_rh:x.donor_rh, qty:qty, pack_no:String(pack_no).trim(), series:String(series).trim()
  });
  toast(r.ok?'✅ Кількість партії скориговано':(r.error||'Помилка коригування'), r.ok?'good':'bad');
...
async function chooseApprovedRequestForComponentV6422(x){
  ...
  if(items.length){
    text='Погоджені вимоги:\n'+items.slice(0,12).map(r=>{
      const line1=`№${r.id} — ${r.patient_name||''} · ${r.component||''} · ${r.patient_group||''}${r.patient_rh||''} · к-сть ${r.amount||''}`;
      const line2=`   Відділення: ${r.department||'—'} · Потрібно: ${r.needed_date||'—'} · Терміновість: ${r.urgency||'—'}`;
      const line3=`   Адреса: ${r.address||'—'} · Діагноз: ${r.diagnosis||'—'}`;
      return line1+'\n'+line2+'\n'+line3;
    }).join('\n')+'\n\nВведіть № вимоги';
  }
...
async function loadAll(){
  // V6.4.40: легке оновлення. Не вантажимо всі 20+ API кожні 5 секунд.
...
// V6.4.40: остаточна видимість розділів у браузері та PWA.
...
(function(){
  const ROLE_SECTIONS_V6410={
    admin:['homeSec','requestsSec','patientsSec','transfusionJournalSec','reactionRegistrySec','signSec','stockSec','alertsSec','qrScannerSec','barcodeSec','traceabilitySec','incompatSec','temperatureSec','writeoffSec','warningsSec','telegramPersonalSec','usersSec','auditSec','reportsSec','backupSec','trashSec','maintenanceSec','pwaInstallSec','dashboardProSec'],
    transfusion:['homeSec','requestsSec','patientsSec','transfusionJournalSec','reactionRegistrySec','signSec','stockSec','alertsSec','qrScannerSec','barcodeSec','traceabilitySec','incompatSec','temperatureSec','writeoffSec','warningsSec','telegramPersonalSec','usersSec','auditSec','reportsSec','maintenanceSec','trashSec','pwaInstallSec','dashboardProSec'],
...
// V6.4.40 FINAL ROLE CLEANUP: прибирає зайві панелі для doctor/nurse у браузері та PWA.
(function(){
  const PERSONAL_SECTIONS={
    doctor:['homeSec','requestsSec','patientsSec','transfusionJournalSec','reactionRegistrySec','signSec','telegramPersonalSec','pwaInstallSec'],
    nurse:['homeSec','requestsSec','patientsSec','transfusionJournalSec','reactionRegistrySec','signSec','telegramPersonalSec','pwaInstallSec']
  };
  const SERVICE_SECTIONS=['stockSec','alertsSec','qrScannerSec','barcodeSec','traceabilitySec','incompatSec','temperatureSec','writeoffSec','dailyReportSec','warningsSec','telegramSec','usersSec','auditSec','reportsSec','backupSec','trashSec','maintenanceSec','dashboardProSec','upgradeV640Sec'];
...
// V6.4.40: doctor/nurse мають право списувати тільки власні погоджені/видані вимоги.
