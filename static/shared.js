...
async function loadHomeStatusV6424(){
  try{ const h=await jget('/api/health'); updateHomeSystemStatusV6424(h); }catch(e){
    updateHomeSystemStatusV6424({ok:false,version:'V6.4.28',database_status_label:'❌ Недоступно',database_type:'—',backup_status_label:'—',telegram_status_label:'—'});
  }
}
...
async function loadTrashV615(){
  ...
  let d={items:[]};
  try{d=await jget('/api/trash')}catch(e){box.innerHTML='<div class="danger">Помилка завантаження кошика: '+esc(e.message||e)+'</div>';return;}
  const items=d.items||[];
