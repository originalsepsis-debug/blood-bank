...
async function loadComponentStockV613(){
  ...
  try{
    res=await jget('/api/component-stock');
    if(!res || res.ok===false) throw new Error((res&&res.error)||'component-stock error');
  }catch(e){
    try{
      const fallback=await jget('/api/stock/summary');
  ...
  if(!res.ok){
    const msg='<div class="danger">Помилка завантаження компонентів: '+escapeHtmlV630(res.error||'невідомо')+'</div>';
    if(cards) cards.innerHTML=msg;
    if(table) table.innerHTML=msg;
    return;
  }
  if(cards){
    cards.innerHTML=items.map(x=>`<div class="comp-card ${componentColorV613(x.component)}"><b>${escapeHtmlV630(x.component)}</b><span>${escapeHtmlV630(x.donor_group)} ${escapeHtmlV630(x.donor_rh)}</span><strong>${x.qty}</strong>${x.nearest_expiry?`<small>Термін: ${escapeHtmlV630(x.nearest_expiry)}</small>`:''}</div>`).join('')||'<div class="notice">Немає компонентів</div>';
  }
  if(table){
    table.innerHTML='<div class="table-scroll"><table><tr><th>Компонент</th><th>Група</th><th>Rh</th><th>К-сть</th><th>Пакетів</th><th>Найближчий термін</th></tr>'+
      items.map(x=>`<tr><td>${escapeHtmlV630(x.component)}</td><td>${escapeHtmlV630(x.donor_group)}</td><td>${escapeHtmlV630(x.donor_rh)}</td><td>${x.qty}</td><td>${escapeHtmlV630(String(x.packs||''))}</td><td>${escapeHtmlV630(x.nearest_expiry||'')}</td></tr>`).join('')+
      '</table></div>';
  }
}
...
async function loadAll(){
  // V6.4.15: легке оновлення. Не вантажимо всі 20+ API кожні 5 секунд.
...
// V6.4.15: остаточна видимість розділів у браузері та PWA.
...
// V6.4.15 FINAL ROLE CLEANUP: прибирає зайві панелі для doctor/nurse у браузері та PWA.
(function(){
