    ...
    entries.forEach(x=>{
      ...
      list+=`<div class="stock-entry-card">
        ...
        <div class="stock-entry-grid">
          ...
          <div><b>К-сть</b><span>${escHtml(x.amount||'')}</span></div>
...
async function findStockEntryV6423(id){
  const r=await jget('/api/stock/entries',{ttl:0,timeout:30000});
...
async function writeoffStockEntryV6423(id){
  ...
  const r=await jpost('/api/component-stock/writeoff',{component:x.component, donor_group:x.donor_group, donor_rh:x.donor_rh, qty, request_id, reason, pack_no:x.pack_no, series:x.series, expiry:x.expiry, stock_entry_id:x.stock_entry_id});
...
async function writeoffComponentStockV6422(idx){
  ...
  const r=await jpost('/api/component-stock/writeoff',{component:x.component, donor_group:x.donor_group, donor_rh:x.donor_rh, qty, request_id, reason, pack_no:x.pack_no||'', series:x.series||'', expiry:x.expiry||''});
