...
async function loadComponentStockV613(force=false){
  ...
  const html=comps.map(comp=>{
    ...
    const rowHtml=rows.map(x=>{
      const idx=window.COMPONENT_STOCK_GROUPS.push({component:comp, donor_group:x.donor_group, donor_rh:x.donor_rh, qty:x.qty, expiry:x.nearest_expiry||''})-1;
...
async function editComponentStockV6417(idx){
  ...
  if(r.ok){ if(window.API_CACHE) window.API_CACHE={}; await loadComponentStockV613(true); if(typeof loadDashboardProV640==='function') loadDashboardProV640(); }
}
...
async function loadAll(){
  // V6.4.17: легке оновлення. Не вантажимо всі 20+ API кожні 5 секунд.
...
// V6.4.17: остаточна видимість розділів у браузері та PWA.
...
// V6.4.17 FINAL ROLE CLEANUP: прибирає зайві панелі для doctor/nurse у браузері та PWA.
(function(){
