...
/* cleaned duplicate function show from old patch lines 45-45 */
...
/* cleaned duplicate function loadAll from old patch lines 77-77 */
...
async function loadStock(){
  ...
  let list='<h3>Записи складу</h3><div class="notice">Тут можна редагувати або видаляти помилково внесені старі записи складу. Видалені записи переносяться в кошик.</div>';
  list+='<div class="table-scroll"><table><tr><th>ID</th><th>Дата</th><th>Тип</th><th>Компонент</th><th>Група</th><th>Rh</th><th>К-сть</th><th>Пакет</th><th>Серія</th><th>Термін</th><th>Дії</th></tr>';
  entries.forEach(x=>{list+=`<tr><td>${x.id}</td><td>${escHtml(x.created_at||'')}</td><td>${escHtml(x.type||'')}</td><td>${escHtml(x.component||'')}</td><td>${escHtml(x.donor_group||'')}</td><td>${escHtml(x.donor_rh||'')}</td><td>${escHtml(x.amount||'')}</td><td>${escHtml(x.pack_no||'')}</td><td>${escHtml(x.series||'')}</td><td>${escHtml(x.expiry||'')}</td><td><button class="btn-blue small-btn" onclick="editStockEntry(${x.id})">Редагувати</button><button class="btn-red small-btn" onclick="deleteStockEntry(${x.id})">Видалити</button></td></tr>`});
  list+='</table></div>';
...
async function editStockEntry(id){
  let r=await jget('/api/stock/entries',{ttl:0}); const x=(r.items||[]).find(i=>Number(i.id)===Number(id)); if(!x){toast('Запис не знайдено','warn');return;}
  let d={id};
...
async function loadReport(){
  ...
  if(!d.ok){box.innerHTML='<div class="danger">Не вдалося сформувати звіт</div>';return;}
  let h=`<div class="good"><b>${d.period.label}</b><br>Період: ${d.period.date_from} — ${d.period.date_to}<br>Сформовано: ${d.period.issued_at}</div>`;
  h+='<div class="table-scroll"><table><tr><th>Компонент</th><th>Залишок поч.</th><th>Одержано</th><th>Використано/видано</th><th>Залишок кін.</th><th>Списано строк</th><th>Списано інше</th></tr>'+
    (d.rows||[]).map(x=>`<tr><td>${x.component}</td><td>${x.opening_total}</td><td>${x.received_total}</td><td>${x.used_total}</td><td>${x.closing_total}</td><td>${x.expired_total}</td><td>${x.other_writeoff_total}</td></tr>`).join('')+'</table></div>';
  h+='<h3>Видано за період</h3><div class="table-scroll"><table><tr><th>Дата видачі</th><th>Пацієнт</th><th>Компонент</th><th>Група</th><th>Rh</th><th>К-сть</th><th>Пакет</th></tr>'+
    (d.issues||[]).slice(0,50).map(x=>`<tr><td>${x['Дата видачі']||''}</td><td>${x['Пацієнт']||''}</td><td>${x['Компонент']||''}</td><td>${x['Група']||''}</td><td>${x['Rh']||''}</td><td>${x['Кількість']||''}</td><td>${x['№ пакета']||''}</td></tr>`).join('')+'</table></div>';
  box.innerHTML=h;
}
...
/* cleaned duplicate function loadTelegramStatus from old patch lines 314-326 */
...
async function loadHealth(){
  ...
  box.innerHTML=`<div class="dashboard-grid">
    ...
    <div class="dashboard-box"><b>База даних</b><br>${h.database}<br>${h.postgres?'PostgreSQL':'SQLite'}</div>
    <div class="dashboard-box"><b>Вік резервної копії</b><br>${h.backup_age_hours??'немає'} год</div>
...
async function createRollbackSnapshot(){
  let r=await jpost('/api/backups/відновлення-snapshot',{});
  toast(r.ok?'✅ Rollback snapshot створено':'⚠️ '+(r.error||'Помилка snapshot'),r.ok?'good':'warn');
  loadHealth();
}

/* cleaned duplicate function loadTelegramMe from old patch lines 375-398 */

/* cleaned duplicate function saveTelegramMeSettings from old patch lines 399-412 */

/* cleaned duplicate function testMyTelegram from old patch lines 413-416 */

/* cleaned duplicate function pollTelegram from old patch lines 417-421 */
...
/* cleaned duplicate function updatePWAStatus from old patch lines 437-448 */
...
/* cleaned duplicate function loadTelegramMe from old patch lines 480-503 */

/* cleaned duplicate function saveTelegramMeSettings from old patch lines 504-516 */

/* cleaned duplicate function testMyTelegram from old patch lines 517-520 */

/* cleaned duplicate function pollTelegram from old patch lines 521-525 */
...
const ROLE_SECTION_MAP={
  admin:["dashboard","stock","requests","reports","users","telegram","telegramPersonal","pwa","monitor","audit","maintenance","barcode","traceability","incompat","dashboardPro","temperature","writeoff","dailyReport","backup"],
  transfusion:["dashboard","stock","requests","reports","users","telegram","telegramPersonal","pwa","monitor","audit","maintenance","barcode","traceability","incompat","dashboardPro","temperature","writeoff","dailyReport"],
...
/* cleaned duplicate function safeOpenFeature from old patch lines 762-778 */


/* cleaned duplicate function openRoleFeature from old patch lines 779-779 */
...
/* cleaned duplicate function loadTelegramStatus from old patch lines 820-832 */

/* cleaned duplicate function loadTelegramMe from old patch lines 833-856 */


/* cleaned duplicate function updatePWAStatus from old patch lines 857-868 */

const oldLoadAllV61 = typeof loadAll === 'function' ? loadAll : null;
/* cleaned duplicate function loadAll from old patch lines 870-873 */
...
/* cleaned duplicate function safeShowV613 from old patch lines 878-885 */

/* cleaned duplicate function applyRoleVisibilityV613 from old patch lines 886-895 */


/* cleaned duplicate function componentColorV613 from old patch lines 896-896 */

/* cleaned duplicate function loadComponentStockV613 from old patch lines 897-905 */
...
/* cleaned duplicate function show from old patch lines 915-920 */

const oldLoadAllV613=typeof loadAll==='function'?loadAll:null;
/* cleaned duplicate function loadAll from old patch lines 922-922 */
...
/* cleaned duplicate function loadComponentStockV613 from old patch lines 928-936 */
...
/* cleaned duplicate function scanManualBarcodeV614 from old patch lines 944-949 */
...
/* cleaned duplicate function componentColorV613 from old patch lines 969-969 */

function groupByV615(arr,key){return arr.reduce((a,x)=>{const k=x[key]||'—';(a[k]=a[k]||[]).push(x);return a;},{});}
/* cleaned duplicate function loadComponentStockV613 from old patch lines 971-979 */
...
/* cleaned duplicate function scanManualBarcodeV614 from old patch lines 1017-1027 */

const oldShowV615 = typeof show==='function'?show:null;

/* cleaned duplicate function show from old patch lines 1029-1042 */
...
/* cleaned duplicate function safeShowV613 from old patch lines 1065-1075 */


/* cleaned duplicate function show from old patch lines 1076-1090 */

const oldApplyRoleVisibilityV616 = typeof applyRoleVisibilityV613==='function'?applyRoleVisibilityV613:null;
/* cleaned duplicate function applyRoleVisibilityV613 from old patch lines 1092-1096 */
...
/* cleaned duplicate function forceShowPatientV617 from old patch lines 1122-1134 */


/* cleaned duplicate function show from old patch lines 1135-1150 */


/* cleaned duplicate function safeShowV613 from old patch lines 1151-1161 */


/* cleaned duplicate function openRoleFeature from old patch lines 1162-1162 */


/* cleaned duplicate function safeOpenFeature from old patch lines 1163-1163 */

const oldApplyRoleVisibilityV617 = typeof applyRoleVisibilityV613==='function'?applyRoleVisibilityV613:null;
/* cleaned duplicate function applyRoleVisibilityV613 from old patch lines 1165-1183 */
...
/* cleaned duplicate function forceShowPatientV617 from old patch lines 1194-1209 */


/* cleaned duplicate function show from old patch lines 1210-1226 */


/* cleaned duplicate function safeShowV613 from old patch lines 1227-1235 */
...
async function loadSectionDataV642(id){
  ...
  try{
    const tasks=[];
    if(id==='homeSec') tasks.push(runIfV642('loadHome'), runIfV642('loadReminders'));
...
async function applyRoleVisibilityV613(){
  ...
  const allowed={
    ...
    transfusion:['homeSec','stockSec','requestsSec','patientsSec','componentsSec','warningsSec','traceabilitySec','temperatureSec','dashboardProSec','barcodeSec','transfusionJournalSec','settingsSec','maintenanceSec','telegramSec','telegramPersonalSec','pwaInstallSec','reportsSec','usersSec','auditSec','reactionRegistrySec','signSec'],
...
async function loadComponentStockV613(force=false){
  ...
  // doctor/nurse не повинні вантажити службовий склад навіть випадково.
  ...
  if(cards && force) cards.innerHTML='<div class="notice">Завантаження компонентів…</div>';

  let res={ok:false,items:[],error:''};
  try{
    // V6.4.16: довший timeout і кеш, бо Render/PostgreSQL іноді відповідає повільно.
    res=await jget('/api/component-stock',{ttl:60000,timeout:45000});
    if(!res || res.ok===false) throw new Error((res&&res.error)||'component-stock error');
  }catch(e){
    const msg=String((e&&e.message)||e||'');
    if(msg.toLowerCase().includes('abort')){
      // Не засмічуємо екран помилкою "signal is aborted". Даємо користувачу кнопку повтору.
      if(cards && !cards.innerHTML.trim()) cards.innerHTML='<div class="notice">Компоненти завантажуються повільно. Натисніть “Оновити компоненти”.</div>';
      if(table) table.innerHTML='<button class="btn-blue" onclick="loadComponentStockV613(true)">Оновити компоненти</button>';
      window.COMPONENT_STOCK_LOADING=false;
      return;
    }
    try{
      const fallback=await jget('/api/stock/summary',{ttl:60000,timeout:45000});
  ...
  const html=comps.map(comp=>{
    ...
    const rowHtml=rows.map(x=>`<div class="component-row"><div><b>Група</b><span>${escapeHtmlV630(x.donor_group)}</span></div><div><b>Rh</b><span>${escapeHtmlV630(x.donor_rh)}</span></div><div><b>К-сть</b><span>${escapeHtmlV630(String(x.qty))}</span></div><div><b>Пакетів</b><span>${escapeHtmlV630(String(x.packs||''))}</span></div><div><b>Найближчий термін</b><span>${escapeHtmlV630(x.nearest_expiry||'—')}</span></div></div>`).join('');
    return `<details class="component-detail ${componentColorV613(comp)}"><summary><span>${escapeHtmlV630(comp)}</span><strong>${sum}</strong>${nearest?`<small>Термін: ${escapeHtmlV630(nearest)}</small>`:''}</summary><div class="component-detail-body">${rowHtml}</div></details>`;
  }).join('');
  ...
  // Внизу більше не дублюємо широку таблицю, бо вона виходила за межі на телефоні.
  if(table) table.innerHTML='';
}
...
async function loadAll(){
  // V6.4.16: легке оновлення. Не вантажимо всі 20+ API кожні 5 секунд.
...
// V6.4.16: остаточна видимість розділів у браузері та PWA.
...
// V6.4.16 FINAL ROLE CLEANUP: прибирає зайві панелі для doctor/nurse у браузері та PWA.
(function(){
