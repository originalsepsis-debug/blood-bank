...
const ROLE_SECTION_MAP={
  ...
  nurse:["dashboard","requests","patients","history","telegram","pwa"]
};
...
async function v60ApplyPermissions(){
  try{
    ...
    document.querySelectorAll('.role-admin-only,.audit-action,a[href*="audit/export"],a[href*="backups"],button[onclick*="Maintenance"],button[onclick*="rollback"]').forEach(el=>{
      el.classList.toggle('role-hidden', !['admin','transfusion'].includes(role));
    });
  }catch(e){}
}
...
async function applyRoleVisibilityV613(){
  ...
  const allowed={
    admin:['homeSec','stockSec','requestsSec','patientsSec','componentsSec','warningsSec','traceabilitySec','temperatureSec','dashboardProSec','barcodeSec','transfusionJournalSec','usersSec','auditSec','trashSec','backupSec','settingsSec','maintenanceSec'],
    transfusion:['homeSec','stockSec','requestsSec','patientsSec','componentsSec','warningsSec','traceabilitySec','temperatureSec','dashboardProSec','barcodeSec','transfusionJournalSec','trashSec','backupSec','settingsSec','maintenanceSec'],
    doctor:['homeSec','requestsSec','patientsSec','transfusionJournalSec'],
    nurse:['homeSec','requestsSec','patientsSec','transfusionJournalSec']
  };
...
async function loadComponentStockV613(){
  ...
  let res={items:[]};
  try{res=await jget('/api/component-stock');}catch(e){}
  const items=res.items||res||[];
  if(cards){cards.innerHTML=items.map(x=>`<div class="comp-card ${componentColorV613(x.component)}"><b>${x.component||''}</b><span>${x.group||x.donor_group||''}${x.rh||x.donor_rh||''}</span><strong>${x.qty||x.amount||0}</strong></div>`).join('')||'<div class="notice">Немає компонентів</div>';}
  if(table){table.innerHTML='<table><tr><th>Компонент</th><th>Група</th><th>Rh</th><th>К-сть</th></tr>'+items.map(x=>`<tr><td>${x.component||''}</td><td>${x.group||x.donor_group||''}</td><td>${x.rh||x.donor_rh||''}</td><td>${x.qty||x.amount||0}</td></tr>`).join('')+'</table>';}
}
...
async function loadTelegramMe(){
  ...
  if(a){a.href=d.connect_url||'#'; a.style.display=d.connect_url?'inline-flex':'none';}
  box.innerHTML=`<div class="setting-list"><label><input type="checkbox" id="tg_set_enabled" ${d.telegram_enabled?'checked':''}> Увімкнути Telegram</label><label><input type="checkbox" id="tg_set_new" ${d.telegram_notify_new_requests?'checked':''}> Нові вимоги</label><label><input type="checkbox" id="tg_set_critical" ${d.telegram_notify_critical?'checked':''}> Критичні залишки</label><label><input type="checkbox" id="tg_set_exp" ${d.telegram_notify_expiring?'checked':''}> Терміни придатності</label><label><input type="checkbox" id="tg_set_react" ${d.telegram_notify_reactions?'checked':''}> Реакції</label><label><input type="checkbox" id="tg_set_backup" ${d.telegram_notify_backups?'checked':''}> Backup</label><button onclick="saveTelegramMeSettings()">Зберегти Telegram</button><button onclick="testMyTelegram()">Тест</button></div>`;
}

async function saveTelegramMeSettings(){
  const get=id=>{const el=document.getElementById(id);return !!(el&&el.checked)};
  const r=await jpost('/api/telegram/me/settings',{telegram_enabled:get('tg_set_enabled'),telegram_notify_new_requests:get('tg_set_new'),telegram_notify_critical:get('tg_set_critical'),telegram_notify_expiring:get('tg_set_exp'),telegram_notify_reactions:get('tg_set_react'),telegram_notify_backups:get('tg_set_backup')});
...
async function loadAll(){
  // V6.4.9: легке оновлення. Не вантажимо всі 20+ API кожні 5 секунд.
...
async function loadUpgradeV640(){
  ...
  for(const ep of endpoints){
    try{ const r=await api(ep); html += `<div class="notice"><b>${ep}</b><pre>${escapeHtml(JSON.stringify(r,null,2))}</pre></div>`; }
    catch(e){ html += `<div class="notice danger"><b>${ep}</b>: ${escapeHtml(String(e.message||e))}</div>`; }
  }
  box.innerHTML=html;
}
async function runScheduledBackupV640(){
  try{ const r=await api('/api/backups/schedule/run',{method:'POST',headers:{'X-CSRF-Token':CSRF,'Content-Type':'application/json'},body:'{}'}); toast('Backup: '+(r.filename||'готово')); loadUpgradeV640(); }
  catch(e){ toast('Помилка backup: '+e.message); }
}
