...
async function loadComponentStockV613(){
  ...
  const items=(res.items||res||[]).filter(x=>Number(x.qty||x.amount||0)!==0);
  if(res && res.ok===false && cards){cards.innerHTML='<div class="danger">'+(res.error||'Немає доступу')+'</div>';}
  if(cards && !(res&&res.ok===false)){cards.innerHTML=items.map(x=>`<div class="comp-card ${componentColorV613(x.component)}"><b>${x.component||''}</b><span>${x.group||x.donor_group||''}${x.rh||x.donor_rh||''}</span><strong>${x.qty||x.amount||0}</strong></div>`).join('')||'<div class="notice">Немає компонентів</div>';}
...
async function loadAll(){
  // V6.4.14: легке оновлення. Не вантажимо всі 20+ API кожні 5 секунд.
...
// V6.4.14: остаточна видимість розділів у браузері та PWA.
...
// V6.4.14 FINAL ROLE CLEANUP: прибирає зайві панелі для doctor/nurse у браузері та PWA.
(function(){
