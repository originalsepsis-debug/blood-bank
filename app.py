...
def health_payload():
    ...
    return {"ok":ok,"version":"V6.4.12","database":"ok" if ok else "error","database_error":err,"postgres":IS_POSTGRES,"telegram_configured":bool(TELEGRAM_BOT_TOKEN and TELEGRAM_CHAT_ID),"backup_age_hours":age,"auto_backup_enabled":AUTO_BACKUP_ENABLED,
...
def api_external_status():
    return jsonify(ok=True, version="V6.4.12", postgres=IS_POSTGRES, time=now())
...
def api_tx_reset_v594():
    db_rollback_safe()
    return jsonify(ok=True, version="V6.4.12", message="transaction rolled back")


@app.get("/api/ui/feature-map")
@login_required
def api_ui_feature_map():
    return jsonify(ok=True, version="V6.4.12", features={
...
def api_version_v618():
    return jsonify(ok=True, version="V6.4.12", title=APP_TITLE)
...
# ================= V6.4.12 PERIODIC MOZ BLOOD COMPONENT REPORTS + STOCK FULL COMPONENTS =================
...
# ================= END V6.4.12 PERIODIC MOZ BLOOD COMPONENT REPORTS + STOCK FULL COMPONENTS =================
