...
def health_payload():
    ...
    return {"ok":ok,"version":"V6.4.8","database":"ok" if ok else "error","database_error":err,"postgres":IS_POSTGRES,"telegram_configured":bool(TELEGRAM_BOT_TOKEN and TELEGRAM_CHAT_ID),"backup_age_hours":age,"auto_backup_enabled":AUTO_BACKUP_ENABLED,
...
def api_backup_download(bid):
    ...
    if not b:
        return jsonify(ok=False, error="Backup не знайдено"), 404
    path = os.path.join(BACKUP_DIR, b["filename"])
    if not os.path.exists(path):
        return jsonify(ok=False, error="Файл backup не знайдено"), 404
...
def api_external_status():
    return jsonify(ok=True, version="V6.4.8", postgres=IS_POSTGRES, time=now())
...
def api_tx_reset_v594():
    db_rollback_safe()
    return jsonify(ok=True, version="V6.4.8", message="transaction rolled back")


@app.get("/api/ui/feature-map")
@login_required
def api_ui_feature_map():
    return jsonify(ok=True, version="V6.4.8", features={
...
ROLE_PERMISSIONS = {
    "admin": {
        "label": "Адміністратор",
        "sections": ["dashboard","stock","requests","reports","users","telegram","pwa","monitor","audit","maintenance","barcode","traceability","incompat","dashboardPro","temperature","writeoff","dailyReport"],
    ...
    "doctor": {
        "label": "Лікар",
        "sections": ["dashboard","requests","patients","history","telegram","pwa"],
...
def api_ui_role_config_v613():
    ...
    if role in ("admin","transfusion"):
        allowed=["home","patients","requests","stock","components","reports","users","telegram","pwa","monitor","maintenance","audit","backup","barcode","traceability","incompat","dashboardPro","temperature","writeoff","dailyReport","transfusionJournal","reactions"]
    elif role=="doctor":
        allowed=["home","patients","requests","myRequests","history","telegramPersonal","pwaPersonal","transfusionJournal","reactions"]
    elif role=="nurse":
        allowed=["home","patients","requests","myRequests","history","telegramPersonal","pwaPersonal","transfusionJournal","reactions"]
    else:
...
def api_component_stock_v623():
    """Compatibility endpoint for the component stock cards in shared.js."""
    try:
        items = rows("""SELECT component, donor_group, donor_rh, donor_group as 'group', donor_rh as rh,
                              SUM(CASE WHEN type='Надходження' THEN amount ELSE -amount END) AS qty,
                              SUM(CASE WHEN type='Надходження' THEN amount ELSE -amount END) AS amount
                       FROM stock_entries
                       GROUP BY component, donor_group, donor_rh
                       ORDER BY component, donor_group, donor_rh""")
        return jsonify(ok=True, items=items)
...
def api_version_v618():
    return jsonify(ok=True, version="V6.4.8", title=APP_TITLE)
...
# ================= V6.4.8 PERIODIC MOZ BLOOD COMPONENT REPORTS + STOCK FULL COMPONENTS =================
...
# ================= END V6.4.8 PERIODIC MOZ BLOOD COMPONENT REPORTS + STOCK FULL COMPONENTS =================
