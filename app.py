...
def _parse_backup_dt(value):
    """V6.4.36: parse backup timestamps from DB rows or file mtime fallback."""
    if not value:
        return None
...
def _latest_backup_info():
    """V6.4.36: show last backup from DB; if record is missing, detect newest ZIP/DB backup file in BACKUP_DIR."""
...
def health_payload():
    ...
    return {
        "ok": ok,
        "version": APP_VERSION if 'APP_VERSION' in globals() else "V6.4.36",
...
def forbid_json():
    return jsonify(ok=False, error="Недостатньо прав"), 403
# ================= END V6.4.36 PERMISSION MATRIX =================

@app.route("/")
def index():
    # V6.4.36 FAST LOGIN: do not run schema migrations on every page open.
...
def login():
    ...
    db_rollback_safe()
    # V6.4.36 FAST LOGIN: migrations are not executed during password submit.
...
def api_request_writeoff():
    # V6.4.36: списання дозволене після погодження трансфузіологом.
...
def _annotate_stock_entry_reconciliation(items):
    """V6.4.36: позначає закриті надходження.
...
def delete_trash_record_permanent():
    """V6.4.36: остаточно стерти запис із кошика.
...
def api_patient_history():
    # V6.4.36: doctor/nurse бачать історію тільки власних вимог, включно зі списаними/відмовленими/використаними.
...
def api_external_status():
    return jsonify(ok=True, version="V6.4.36", postgres=IS_POSTGRES, time=now())
...
def _traceability_rows_for_code(code):
    """V6.4.36: простежуваність має працювати не тільки з package_traceability.
...
def api_writeoff_component():
    """V6.4.36: списання через окремий розділ має реально зменшувати склад.
...
def api_public_health_v593():
    # V6.4.36: легка публічна перевірка без міграцій і без розкриття службових даних.
    out={"ok":False,"version":"V6.4.36"}
    try:
        row("SELECT 1 AS ok")
        out["ok"]=True
...
def api_tx_reset_v594():
    db_rollback_safe()
    return jsonify(ok=True, version="V6.4.36", message="transaction rolled back")


@app.get("/api/ui/feature-map")
@login_required
def api_ui_feature_map():
    return jsonify(ok=True, version="V6.4.36", features={
...
@app.get("/api/health")
def api_health_v60():
    # V6.4.36: lightweight real home status. No migrations on every health call.
    out = health_payload()
    try:
        out["db_select"] = row("SELECT 1 AS ok")
    except Exception as e:
...
def api_ui_role_config_v613():
    ...
    if role in ("admin","transfusion"):
        ...
        # V6.4.36: doctor має бачити тільки робочі персональні розділи, без складу/моніторингу/системи
        allowed=["home","patients","requests","myRequests","history","telegramPersonal","pwa","transfusionJournal","reactions","sign"]
    elif role=="nurse":
        # V6.4.36: nurse має бачити тільки робочі персональні розділи, без складу/моніторингу/системи
        allowed=["home","patients","requests","myRequests","history","telegramPersonal","pwa","transfusionJournal","reactions","sign"]
    else:
...
def api_stock_summary_v6415():
    """V6.4.36: підсумок складу для деталей компонентів. Працює однаково в SQLite/PostgreSQL."""
    try:
        delta=_stock_delta_sql()
        items=[]
        try:
...
def api_trash_v615_final():
    """V6.4.36: кошик показує реальні записи з таблиці trash + старі записи зі status deleted/trash.
...
def api_component_stock_v6415():
    """V6.4.36: єдине джерело для блоку «Компоненти на складі».
...
def api_component_stock_writeoff_v6422():
    ...
    pack_no=(d.get('pack_no') or (stock_rec or {}).get('pack_no') or (unit or {}).get('pack_no') or '').strip()
    series=(d.get('series') or (stock_rec or {}).get('series') or (unit or {}).get('series') or '').strip()
    # V6.4.36: для списання через розділ «Компоненти/Склад» серія обов’язкова,
...
def api_version_v618():
    return jsonify(ok=True, version="V6.4.36", title=APP_TITLE)
...
# ================= V6.4.36 PERIODIC MOZ BLOOD COMPONENT REPORTS + STOCK FULL COMPONENTS =================
...
def moz_build_report_data(start, end):
    """V6.4.36: strict audited report builder.

    Official summary is calculated only from valid stock movements in stock_entries.
    Invalid movement groups that produce negative closing balances are excluded from official totals
    and moved to warnings. This prevents old/phantom rows (for example platelets that are not in the
    current stock/components list) from polluting the report.
    """
    ...
    # First pass: build raw metrics and detail rows from active stock_entries only.
    for x in stock:
        if is_deleted_or_trash(x):
            continue
        kind=_moz_type_kind(x.get('type'))
        ...
        if not component:
            ...
            continue
        cat=moz_component_category(component)
        ng=moz_norm_group(group); nr=moz_norm_rh(rh)
        if not ng or not nr or (ng,nr) not in MOZ_GROUP_COLUMNS:
            audit_warnings.append({'id':x.get('id'), 'component':component, 'group':group, 'rh':rh, 'message':'Запис не врахований: відсутня або неправильна група/Rh'})
            continue

        def add(metric, val):
            _moz_add_with_fallback(summary, component, group, rh, metric, val, code)

        if _is_before(created, start):
            if kind=='in':
                add('opening', amount)
            elif kind in ('issue','writeoff'):
                add('opening', -amount)
        if _in_period(created, start, end):
            if kind=='in':
                ...
                issue_rows.append({'_key':(cat,ng,nr), 'Дата видачі': created, 'Пацієнт': (req or {}).get('patient_name','') if req else x.get('patient_name',''), 'Відділення': (req or {}).get('department','') if req else '', 'Компонент': cat, 'Група': ng, 'Rh': nr, 'Кількість': amount, '№ пакета': x.get('pack_no',''), 'Серія': x.get('series',''), 'Термін': x.get('expiry',''), 'Примітка': x.get('note','')})
            elif kind=='writeoff':
                note=_txt(x.get('note')).lower()
                ...
                add(metric, amount)
                req=_moz_request_for_stock(x)
                writeoff_rows.append({'_key':(cat,ng,nr), 'Дата списання': created, 'Пацієнт': (req or {}).get('patient_name','') if req else x.get('patient_name',''), 'Відділення': (req or {}).get('department','') if req else '', 'Компонент': cat, 'Група': ng, 'Rh': nr, 'Кількість': amount, '№ пакета': x.get('pack_no',''), 'Серія': x.get('series',''), 'Термін': x.get('expiry',''), 'Причина': x.get('note','')})

    invalid_keys=set()
    for comp in list(summary.keys()):
        for key, m in summary[comp].items():
            raw=(m['opening'] or 0)+(m['received'] or 0)-(m['used'] or 0)-(m['expired'] or 0)-(m['other_writeoff'] or 0)
            m['closing_raw']=raw
            if raw < -0.000001:
                invalid_keys.add((comp,key[0],key[1]))
                audit_warnings.append({'component':comp, 'group':key[0], 'rh':key[1], 'message':f'Негативний залишок {raw:g}. Рухи цієї групи виключені з підсумку звіту. Перевірте старі записи надходження/видачі/списання.'})
                # Do not let phantom/old corrupted rows distort official report totals.
                for k in ['opening','received','used','expired','other_writeoff']:
                    m[k]=0.0
                m['closing']=0.0
            else:
                m['closing']=raw

    # Hide detail rows that belong to invalid excluded groups so preview and summary match.
    issue_rows=[{k:v for k,v in r.items() if k!='_key'} for r in issue_rows if r.get('_key') not in invalid_keys]
    writeoff_rows=[{k:v for k,v in r.items() if k!='_key'} for r in writeoff_rows if r.get('_key') not in invalid_keys]
...
# ================= END V6.4.36 PERIODIC MOZ BLOOD COMPONENT REPORTS + STOCK FULL COMPONENTS =================
