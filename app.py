...
def health_payload():
    ...
    return {"ok":ok,"version":"V6.4.11","database":"ok" if ok else "error","database_error":err,"postgres":IS_POSTGRES,"telegram_configured":bool(TELEGRAM_BOT_TOKEN and TELEGRAM_CHAT_ID),"backup_age_hours":age,"auto_backup_enabled":AUTO_BACKUP_ENABLED,
    ...
    try:
        ...
        if text.startswith("/requests"):
            if is_staff:
                ...
                title = "📋 <b>Останні вимоги</b>"
            elif tg_user:
                req = rows("SELECT id,patient_name,department,component,status FROM requests WHERE created_by=? ORDER BY id DESC LIMIT 10", (tg_user.get("id"),))
                title = "📋 <b>Мої вимоги</b>"
            else:
                telegram_send_message("⛔ Спочатку підключіть Telegram через кнопку в програмі.", "command", chat_id=chat_id, force=True)
                return "not_linked"
            lines = [title]
            for r in req:
...
def telegram_allowed_notifications_for_role(role):
    ...
    # doctor/nurse: only personal request-status style notifications
    return ["new_requests"]
...
def api_telegram_me_settings():
    try:
        ...
        d=request.json or {}
        # Заборонені для ролі сповіщення примусово вимикаються на сервері, не тільки у UI.
...
def api_tx_reset_v594():
    db_rollback_safe()
    return jsonify(ok=True, version="V6.4.11", message="transaction rolled back")


@app.get("/api/ui/feature-map")
@login_required
def api_ui_feature_map():
    return jsonify(ok=True, version="V6.4.11", features={
...
ROLE_PERMISSIONS = {
    ...
    "doctor": {
        "label": "Лікар",
        "sections": ["dashboard","requests","patients","history","telegramPersonal","pwa"],
...
def api_version_v618():
    return jsonify(ok=True, version="V6.4.11", title=APP_TITLE)
...
# ================= V6.4.11 PERIODIC MOZ BLOOD COMPONENT REPORTS + STOCK FULL COMPONENTS =================
...
# ================= END V6.4.11 PERIODIC MOZ BLOOD COMPONENT REPORTS + STOCK FULL COMPONENTS =================
