...
def _request_is_locked_to_other_unit(req, unit):
    """V6.4.65: якщо вимога вже зарезервована під unit_id/pack/series,
...
def health_payload():
    ...
    return {
        "ok": ok,
        "version": APP_VERSION if 'APP_VERSION' in globals() else "V6.4.65",
...
def api_request_update_v6414():
    ...
    allowed={}
    all_fields=["patient_name","birth_date","address","patient_status","department","component","patient_group","patient_rh","amount","urgency","diagnosis","note","status","donor_group","donor_rh","pack_no","series","expiry"]
    for k in all_fields:
        if k in d: allowed[k]=d.get(k)

    critical={"status","component","amount","patient_group","patient_rh","donor_group","donor_rh","pack_no","series","expiry","unit_id"}
...
def api_request_action():
    ...
    if action=="approve":
        ...
        audit("request_reserve", f"request={rid}; unit={unit.get('id')}; qty={qty_res:g}; lot_balance={bal_res:g}")
...
def api_request_writeoff():
    """V6.4.65: не плутати анулювання вимоги зі списанням компонента.
    ...
    # V6.4.65: після списання виданого компонента статус blood_units не має
...
def api_external_status():
    return jsonify(ok=True, version="V6.4.65", postgres=IS_POSTGRES, time=now())
...
def api_health_postgres_v655():
    ...
    try:
        ...
        ok=not errors and all(v is True for v in tables.values() if not isinstance(v,str))
        return jsonify(ok=ok, version=APP_VERSION, postgres=IS_POSTGRES, database_url_set=bool(DATABASE_URL), tables=tables, errors=errors, db_select=row("SELECT 1 AS ok"), token_modes=["X-API-Token","?token="]), (200 if ok else 500)
    except Exception as e:
        db_rollback_safe()
...
def api_tx_reset_v594():
    db_rollback_safe()
    return jsonify(ok=True, version="V6.4.65", message="transaction rolled back")


@app.get("/api/ui/feature-map")
@login_required
def api_ui_feature_map():
    return jsonify(ok=True, version="V6.4.65", features={
...
def _approved_requests_for_component(component='', donor_group='', donor_rh=''):
    ...
    ph=','.join(['?']*len(sts))
    params=list(sts)
    q=f"""SELECT id,created_at,patient_name,birth_date,patient_status,address,department,component,patient_group,patient_rh,amount,urgency,diagnosis,note,status,created_by,doctor_name,doctor_position,compatibility_ok,compatibility_warning,approved_by,issued_by,issued_at,pack_no,series,expiry
...
def api_version_v618():
    return jsonify(ok=True, version="V6.4.65", title=APP_TITLE)
