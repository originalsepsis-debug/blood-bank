...
def _parse_backup_dt(value):
    """V6.4.32: parse backup timestamps from DB rows or file mtime fallback."""
    if not value:
        return None
...
def _latest_backup_info():
    """V6.4.32: show last backup from DB; if record is missing, detect newest ZIP/DB backup file in BACKUP_DIR."""
...
def health_payload():
    ...
    return {
        "ok": ok,
        "version": APP_VERSION if 'APP_VERSION' in globals() else "V6.4.32",
...
def forbid_json():
    return jsonify(ok=False, error="Недостатньо прав"), 403
# ================= END V6.4.32 PERMISSION MATRIX =================

@app.route("/")
def index():
    # V6.4.32 FAST LOGIN: do not run schema migrations on every page open.
...
def login():
    ...
    db_rollback_safe()
    # V6.4.32 FAST LOGIN: migrations are not executed during password submit.
...
def api_request_writeoff():
    # V6.4.32: списання дозволене після погодження трансфузіологом.
...
def delete_trash_record_permanent():
    """V6.4.32: остаточно стерти запис із кошика.
...
def api_patient_history():
    # V6.4.32: doctor/nurse бачать історію тільки власних вимог, включно зі списаними/відмовленими/використаними.
...
def api_external_status():
    return jsonify(ok=True, version="V6.4.32", postgres=IS_POSTGRES, time=now())
...
def _traceability_rows_for_code(code):
    """V6.4.32: простежуваність має працювати не тільки з package_traceability.
...
def api_writeoff_component():
    """V6.4.32: списання через окремий розділ має реально зменшувати склад.
...
def api_public_health_v593():
    # V6.4.32: легка публічна перевірка без міграцій і без розкриття службових даних.
    out={"ok":False,"version":"V6.4.32"}
    try:
        row("SELECT 1 AS ok")
        out["ok"]=True
...
def api_tx_reset_v594():
    db_rollback_safe()
    return jsonify(ok=True, version="V6.4.32", message="transaction rolled back")


@app.get("/api/ui/feature-map")
@login_required
def api_ui_feature_map():
    return jsonify(ok=True, version="V6.4.32", features={
...
@app.get("/api/health")
def api_health_v60():
    # V6.4.32: lightweight real home status. No migrations on every health call.
    out = health_payload()
    try:
        out["db_select"] = row("SELECT 1 AS ok")
    except Exception as e:
...
def api_ui_role_config_v613():
    ...
    if role in ("admin","transfusion"):
        ...
        # V6.4.32: doctor має бачити тільки робочі персональні розділи, без складу/моніторингу/системи
        allowed=["home","patients","requests","myRequests","history","telegramPersonal","pwa","transfusionJournal","reactions","sign"]
    elif role=="nurse":
        # V6.4.32: nurse має бачити тільки робочі персональні розділи, без складу/моніторингу/системи
        allowed=["home","patients","requests","myRequests","history","telegramPersonal","pwa","transfusionJournal","reactions","sign"]
    else:
...
def api_stock_summary_v6415():
    """V6.4.32: підсумок складу для деталей компонентів. Працює однаково в SQLite/PostgreSQL."""
    try:
        delta=_stock_delta_sql()
        items=[]
        try:
...
def api_trash_v615_final():
    """V6.4.32: кошик показує реальні записи з таблиці trash + старі записи зі status deleted/trash.
...
def api_component_stock_v6415():
    """V6.4.32: єдине джерело для блоку «Компоненти на складі».
...
def api_component_stock_writeoff_v6422():
    ...
    pack_no=(d.get('pack_no') or (stock_rec or {}).get('pack_no') or (unit or {}).get('pack_no') or '').strip()
    series=(d.get('series') or (stock_rec or {}).get('series') or (unit or {}).get('series') or '').strip()
    # V6.4.32: для списання через розділ «Компоненти/Склад» серія обов’язкова,
...
def api_version_v618():
    return jsonify(ok=True, version="V6.4.32", title=APP_TITLE)
...
# ================= V6.4.32 PERIODIC MOZ BLOOD COMPONENT REPORTS + STOCK FULL COMPONENTS =================
...
def moz_build_report_data(start, end):
    ...
    for x in stock:
        ...
        is_in = typ in ['надходження','in','income','add','plus','прихід']
        is_issue = typ in ['видача','видано','issue','issued','out']
        is_writeoff = typ in ['списання','writeoff','written_off','minus']
        if _is_before(created, start):
            if is_in: _add_metric(summary, component, group, rh, 'opening', amount)
            elif is_issue or is_writeoff: _add_metric(summary, component, group, rh, 'opening', -amount)
        if _in_period(created, start, end):
            if is_in:
                _add_metric(summary, component, group, rh, 'received', amount)
            elif is_issue:
                _add_metric(summary, component, group, rh, 'used', amount)
                issue_rows.append({
                    'Дата видачі': created, 'Пацієнт': x.get('patient_name',''), 'Відділення': '',
                    'Компонент': component, 'Група': moz_norm_group(group), 'Rh': moz_norm_rh(rh),
                    'Кількість': amount, '№ пакета': x.get('pack_no',''), 'Серія': x.get('series',''),
                    'Термін': x.get('expiry',''), 'Примітка': x.get('note','')
                })
            elif is_writeoff:
    ...
    # Додаткове джерело списань: component_writeoffs.
    # Використовується для старих списань, які могли не створити коректний рух stock_entries.
    try:
        ...
        for w in wrows:
            ...
            if amount<=0:
                continue
            # Якщо такий пакет/серія вже має stock_entries типу Списання у періоді — не дублюємо.
            code=w.get('package_code') or ''
            duplicated=False
            if code:
                try:
                    dr=row(f"""SELECT COUNT(*) AS c FROM stock_entries
                             WHERE (qr_code=? OR pack_no=? OR series=?)
                               AND LOWER(COALESCE(type,'')) IN ('списання','списано','writeoff','written_off')
                               AND created_at>=? AND created_at<=?""", (code,code,code,start.isoformat(),end.isoformat()+" 23:59:59"))
                    duplicated=int((dr or {}).get('c') or 0)>0
                except Exception:
                    db_rollback_safe()
            if not duplicated:
                note=_txt((w.get('reason') or '') + ' ' + (w.get('notes') or '')).lower()
                metric='expired' if ('термін' in note or 'строк' in note or 'придат' in note or 'expire' in note or 'закін' in note) else 'other_writeoff'
                _add_metric(summary, component, '', '', metric, amount)
    except Exception:
        db_rollback_safe()
    ...
    # доповнюємо деталізацію видачі з requests, якщо там є issued_at, але немає рядка stock_entries
    try:
        reqs=rows("SELECT id,issued_at,used_at,patient_name,department,component,amount,donor_group,donor_rh,pack_no,series,expiry,status FROM requests WHERE issued_at IS NOT NULL OR status IN ('Видано','Використано') ORDER BY id DESC")
        ...
        for r in reqs:
            ...
            if _in_period(dt,start,end) and key not in existing:
                issue_rows.append({
                    'Дата видачі': dt, 'Пацієнт': r.get('patient_name',''), 'Відділення': r.get('department',''),
                    'Компонент': r.get('component',''), 'Група': moz_norm_group(r.get('donor_group','')), 'Rh': moz_norm_rh(r.get('donor_rh','')),
                    'Кількість': r.get('amount') or 0, '№ пакета': r.get('pack_no',''), 'Серія': r.get('series',''),
                    'Термін': r.get('expiry',''), 'Примітка': f"Вимога №{r.get('id')} · {r.get('status','')}"
                })
    except Exception:
        db_rollback_safe()
...
def api_reports_moz_period_summary_v643():
    ...
    summary, issues=moz_build_report_data(start,end)
    return jsonify(ok=True, period={'type':ptype,'date_from':start.isoformat(),'date_to':end.isoformat(),'label':label,'issued_at':now()}, rows=moz_flat_summary(summary), issues=issues[:200])
...
def report_moz_period_pdf_v643():
    start,end,label,ptype=moz_period_from_request()
    summary,issues=moz_build_report_data(start,end)
    bio=BytesIO(); c=canvas.Canvas(bio,pagesize=A4); setup_pdf_font(c,9)
    y=800; pdf_text(c,40,y,'Звіт про отримання, використання та списання донорської крові та компонентів крові'); y-=18
    pdf_text(c,40,y,label); y-=18
    pdf_text(c,40,y,f'Період: {start.strftime("%d.%m.%Y")} – {end.strftime("%d.%m.%Y")} · Сформовано: {now()}'); y-=26
    pdf_text(c,40,y,'Компонент | Залишок поч. | Одержано | Використано/видано | Залишок кін. | Списано строк | Списано інше'); y-=14
    for r in moz_flat_summary(summary):
        line=f"{r['component'][:34]} | {r['opening_total']:g} | {r['received_total']:g} | {r['used_total']:g} | {r['closing_total']:g} | {r['expired_total']:g} | {r['other_writeoff_total']:g}"
        pdf_text(c,40,y,line); y-=13
        if y<60:
            c.showPage(); setup_pdf_font(c,9); y=800
    c.save(); bio.seek(0)
    return send_file(bio, as_attachment=True, download_name=f"moz_blood_components_{start.isoformat()}_{end.isoformat()}.pdf")
# ================= END V6.4.32 PERIODIC MOZ BLOOD COMPONENT REPORTS + STOCK FULL COMPONENTS =================
