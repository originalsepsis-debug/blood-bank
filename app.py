...
def _parse_backup_dt(value):
    """V6.4.28: parse backup timestamps from DB rows or file mtime fallback."""
    if not value:
        return None
...
def _latest_backup_info():
    """V6.4.28: show last backup from DB; if record is missing, detect newest ZIP/DB backup file in BACKUP_DIR."""
...
def health_payload():
    ...
    return {
        "ok": ok,
        "version": APP_VERSION if 'APP_VERSION' in globals() else "V6.4.28",
...
def forbid_json():
    return jsonify(ok=False, error="Недостатньо прав"), 403
# ================= END V6.4.28 PERMISSION MATRIX =================

@app.route("/")
def index():
    # V6.4.28 FAST LOGIN: do not run schema migrations on every page open.
...
def login():
    ...
    db_rollback_safe()
    # V6.4.28 FAST LOGIN: migrations are not executed during password submit.
...
def api_request_delete_v6414():
    ...
    execute("INSERT INTO trash(created_at,source_table,source_id,data,deleted_by,reason) VALUES(?,?,?,?,?,?)", (now(),"requests",rid,json.dumps(req,ensure_ascii=False),u.get("username"),d.get("reason") or "Видалено вимогу"))
    if req.get("unit_id"):
...
def api_request_writeoff():
    # V6.4.28: списання дозволене після погодження трансфузіологом.
...
def api_stock():
    ...
    return jsonify(rows(f"""SELECT component,donor_group as 'group',donor_rh as rh,
...
def api_stock_delete_v6414():
    ...
    rec=row("SELECT * FROM stock_entries WHERE id=?", (sid,))
    if not rec: return jsonify(ok=False,error="Запис складу не знайдено"),404
    execute("INSERT INTO trash(created_at,source_table,source_id,data,deleted_by,reason) VALUES(?,?,?,?,?,?)", (now(),"stock_entries",sid,json.dumps(rec,ensure_ascii=False),u.get("username"),d.get("reason") or "Видалено зі складу"))
...
def delete_trash_record_permanent():
    """V6.4.28: остаточно стерти запис із кошика.
...
def api_patient_history():
    # V6.4.28: doctor/nurse бачать історію тільки власних вимог, включно зі списаними/відмовленими/використаними.
...
def api_external_status():
    return jsonify(ok=True, version="V6.4.28", postgres=IS_POSTGRES, time=now())
...
def api_traceability_short_v623(code):
    """Short alias used by the barcode scanner UI."""
    ensure_traceability_tables()
    items = rows("SELECT * FROM package_traceability WHERE package_code=? ORDER BY id DESC LIMIT 200", (code,))
...
def _stock_delta_sql(alias=""):
    ...
    t = f"COALESCE({prefix}type,'')"
    a = f"COALESCE(CAST({prefix}amount AS REAL),0)"
    return f"CASE WHEN {t} IN ('Надходження','in','income','add','plus','+') THEN {a} WHEN {t} IN ('Списання','Видача','Видано','out','writeoff','minus','-') THEN -ABS({a}) ELSE {a} END"
...
def api_tx_reset_v594():
    db_rollback_safe()
    return jsonify(ok=True, version="V6.4.28", message="transaction rolled back")


@app.get("/api/ui/feature-map")
@login_required
def api_ui_feature_map():
    return jsonify(ok=True, version="V6.4.28", features={
...
@app.get("/api/health")
def api_health_v60():
    # V6.4.28: lightweight real home status. No migrations on every health call.
    out = health_payload()
    try:
        out["db_select"] = row("SELECT 1 AS ok")
    except Exception as e:
...
def api_ui_role_config_v613():
    ...
    if role in ("admin","transfusion"):
        ...
        # V6.4.28: doctor має бачити тільки робочі персональні розділи, без складу/моніторингу/системи
        allowed=["home","patients","requests","myRequests","history","telegramPersonal","pwa","transfusionJournal","reactions","sign"]
    elif role=="nurse":
        # V6.4.28: nurse має бачити тільки робочі персональні розділи, без складу/моніторингу/системи
        allowed=["home","patients","requests","myRequests","history","telegramPersonal","pwa","transfusionJournal","reactions","sign"]
    else:
...
def api_stock_summary_v6415():
    """V6.4.28: підсумок складу для деталей компонентів. Працює однаково в SQLite/PostgreSQL."""
    try:
        delta=_stock_delta_sql()
        items=[]
        try:
...
def api_trash_v615_final():
    """V6.4.28: кошик показує реальні записи з таблиці trash + старі записи зі status deleted/trash.
...
def api_component_stock_v6415():
    """V6.4.28: єдине джерело для блоку «Компоненти на складі».
...
def api_component_stock_writeoff_v6422():
    ...
    pack_no=(d.get('pack_no') or (stock_rec or {}).get('pack_no') or (unit or {}).get('pack_no') or '').strip()
    series=(d.get('series') or (stock_rec or {}).get('series') or (unit or {}).get('series') or '').strip()
    # V6.4.28: для списання через розділ «Компоненти/Склад» серія обов’язкова,
    ...
    try:
        ...
        for rec in recs:
            execute("INSERT INTO trash(created_at,source_table,source_id,data,deleted_by,reason) VALUES(?,?,?,?,?,?)",
                    (now(), "stock_entries", rec.get('id'), json.dumps(rec,ensure_ascii=False), u.get('username'), reason))
...
def api_version_v618():
    return jsonify(ok=True, version="V6.4.28", title=APP_TITLE)
...
# ================= V6.4.28 PERIODIC MOZ BLOOD COMPONENT REPORTS + STOCK FULL COMPONENTS =================
...
# ================= END V6.4.28 PERIODIC MOZ BLOOD COMPONENT REPORTS + STOCK FULL COMPONENTS =================
