...
def _parse_backup_dt(value):
    """V6.4.29: parse backup timestamps from DB rows or file mtime fallback."""
    if not value:
        return None
...
def _latest_backup_info():
    """V6.4.29: show last backup from DB; if record is missing, detect newest ZIP/DB backup file in BACKUP_DIR."""
...
def health_payload():
    ...
    return {
        "ok": ok,
        "version": APP_VERSION if 'APP_VERSION' in globals() else "V6.4.29",
...
def forbid_json():
    return jsonify(ok=False, error="Недостатньо прав"), 403
# ================= END V6.4.29 PERMISSION MATRIX =================

@app.route("/")
def index():
    # V6.4.29 FAST LOGIN: do not run schema migrations on every page open.
...
def login():
    ...
    db_rollback_safe()
    # V6.4.29 FAST LOGIN: migrations are not executed during password submit.
...
def api_request_writeoff():
    # V6.4.29: списання дозволене після погодження трансфузіологом.
...
def delete_trash_record_permanent():
    """V6.4.29: остаточно стерти запис із кошика.
...
def api_patient_history():
    # V6.4.29: doctor/nurse бачать історію тільки власних вимог, включно зі списаними/відмовленими/використаними.
...
def api_external_status():
    return jsonify(ok=True, version="V6.4.29", postgres=IS_POSTGRES, time=now())
...
def api_tx_reset_v594():
    db_rollback_safe()
    return jsonify(ok=True, version="V6.4.29", message="transaction rolled back")


@app.get("/api/ui/feature-map")
@login_required
def api_ui_feature_map():
    return jsonify(ok=True, version="V6.4.29", features={
...
@app.get("/api/health")
def api_health_v60():
    # V6.4.29: lightweight real home status. No migrations on every health call.
    out = health_payload()
    try:
        out["db_select"] = row("SELECT 1 AS ok")
    except Exception as e:
...
def api_ui_role_config_v613():
    ...
    if role in ("admin","transfusion"):
        ...
        # V6.4.29: doctor має бачити тільки робочі персональні розділи, без складу/моніторингу/системи
        allowed=["home","patients","requests","myRequests","history","telegramPersonal","pwa","transfusionJournal","reactions","sign"]
    elif role=="nurse":
        # V6.4.29: nurse має бачити тільки робочі персональні розділи, без складу/моніторингу/системи
        allowed=["home","patients","requests","myRequests","history","telegramPersonal","pwa","transfusionJournal","reactions","sign"]
    else:
...
def api_stock_summary_v6415():
    """V6.4.29: підсумок складу для деталей компонентів. Працює однаково в SQLite/PostgreSQL."""
    try:
        delta=_stock_delta_sql()
        items=[]
        try:
...
def api_trash_v615_final():
    """V6.4.29: кошик показує реальні записи з таблиці trash + старі записи зі status deleted/trash.
...
def api_component_stock_v6415():
    """V6.4.29: єдине джерело для блоку «Компоненти на складі».
...
def api_component_stock_writeoff_v6422():
    ...
    pack_no=(d.get('pack_no') or (stock_rec or {}).get('pack_no') or (unit or {}).get('pack_no') or '').strip()
    series=(d.get('series') or (stock_rec or {}).get('series') or (unit or {}).get('series') or '').strip()
    # V6.4.29: для списання через розділ «Компоненти/Склад» серія обов’язкова,
...
def api_version_v618():
    return jsonify(ok=True, version="V6.4.29", title=APP_TITLE)
...
# ================= V6.4.29 PERIODIC MOZ BLOOD COMPONENT REPORTS + STOCK FULL COMPONENTS =================
...
# ================= END V6.4.29 PERIODIC MOZ BLOOD COMPONENT REPORTS + STOCK FULL COMPONENTS =================
