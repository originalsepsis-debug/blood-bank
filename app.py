...
def _parse_backup_dt(value):
    """V6.4.33: parse backup timestamps from DB rows or file mtime fallback."""
    if not value:
        return None
...
def _latest_backup_info():
    """V6.4.33: show last backup from DB; if record is missing, detect newest ZIP/DB backup file in BACKUP_DIR."""
...
def health_payload():
    ...
    return {
        "ok": ok,
        "version": APP_VERSION if 'APP_VERSION' in globals() else "V6.4.33",
...
def forbid_json():
    return jsonify(ok=False, error="Недостатньо прав"), 403
# ================= END V6.4.33 PERMISSION MATRIX =================

@app.route("/")
def index():
    # V6.4.33 FAST LOGIN: do not run schema migrations on every page open.
...
def login():
    ...
    db_rollback_safe()
    # V6.4.33 FAST LOGIN: migrations are not executed during password submit.
...
def api_request_writeoff():
    # V6.4.33: списання дозволене після погодження трансфузіологом.
...
def api_stock_entries_v6414():
    return jsonify(ok=True, items=rows("""SELECT id,created_at,type,component,donor_group,donor_rh,amount,pack_no,series,expiry,patient_name,note,qr_code,unit_id
...
def api_stock_delete_v6414():
    ...
    rec=row("SELECT * FROM stock_entries WHERE id=?", (sid,))
    if not rec: return jsonify(ok=False,error="Запис складу не знайдено"),404
    execute("INSERT INTO trash(created_at,source_table,source_id,data,deleted_by,reason) VALUES(?,?,?,?,?,?)", (now(),"stock_entries",sid,json.dumps(dict(rec),ensure_ascii=False),u.get("username"),d.get("reason") or "Видалено зі складу"))
    execute("DELETE FROM stock_entries WHERE id=?", (sid,))
    if rec.get("unit_id"):
        execute("UPDATE blood_units SET status='deleted', updated_at=?, note=? WHERE id=?", (now(), "Видалено разом із записом складу", int(rec.get("unit_id"))))
        unit_event(rec.get("unit_id"), "stock_entry_deleted", f"Видалено запис складу №{sid}")
    audit("stock_delete", f"stock_entries:{sid}")
    return jsonify(ok=True)
...
def delete_trash_record_permanent():
    """V6.4.33: остаточно стерти запис із кошика.
...
def api_patient_history():
    # V6.4.33: doctor/nurse бачать історію тільки власних вимог, включно зі списаними/відмовленими/використаними.
...
def api_external_status():
    return jsonify(ok=True, version="V6.4.33", postgres=IS_POSTGRES, time=now())
...
def _traceability_rows_for_code(code):
    """V6.4.33: простежуваність має працювати не тільки з package_traceability.
...
def _stock_delta_sql(alias=""):
    ...
    Важливо: типи рухів у старих версіях могли бути різним регістром
    або різними словами (Списано/Видано/Видача). Тому нормалізуємо LOWER().
    """
    prefix = (alias + ".") if alias else ""
    t = f"LOWER(COALESCE({prefix}type,''))"
    a = f"COALESCE(CAST({prefix}amount AS REAL),0)"
    return (
        f"CASE "
        f"WHEN {t} IN ('надходження','in','income','add','plus','+','прихід') THEN {a} "
        f"WHEN {t} IN ('списання','списано','видача','видано','out','issue','issued','writeoff','written_off','minus','-') THEN -ABS({a}) "
        f"ELSE {a} END"
    )

def _active_request_where(alias=""):
    prefix = (alias + ".") if alias else ""
    st = f"LOWER(COALESCE({prefix}status,''))"
    return f"({st} NOT LIKE '%відмов%' AND {st} NOT LIKE '%спис%' AND {st} NOT LIKE '%використ%' AND {st} NOT LIKE '%закрит%' AND {st} NOT LIKE '%deleted%' AND {st} NOT LIKE '%trash%')"
...
def api_writeoff_component():
    """V6.4.33: списання через окремий розділ має реально зменшувати склад.
...
def api_public_health_v593():
    # V6.4.33: легка публічна перевірка без міграцій і без розкриття службових даних.
    out={"ok":False,"version":"V6.4.33"}
    try:
        row("SELECT 1 AS ok")
        out["ok"]=True
...
def api_tx_reset_v594():
    db_rollback_safe()
    return jsonify(ok=True, version="V6.4.33", message="transaction rolled back")


@app.get("/api/ui/feature-map")
@login_required
def api_ui_feature_map():
    return jsonify(ok=True, version="V6.4.33", features={
...
@app.get("/api/health")
def api_health_v60():
    # V6.4.33: lightweight real home status. No migrations on every health call.
    out = health_payload()
    try:
        out["db_select"] = row("SELECT 1 AS ok")
    except Exception as e:
...
def api_ui_role_config_v613():
    ...
    if role in ("admin","transfusion"):
        ...
        # V6.4.33: doctor має бачити тільки робочі персональні розділи, без складу/моніторингу/системи
        allowed=["home","patients","requests","myRequests","history","telegramPersonal","pwa","transfusionJournal","reactions","sign"]
    elif role=="nurse":
        # V6.4.33: nurse має бачити тільки робочі персональні розділи, без складу/моніторингу/системи
        allowed=["home","patients","requests","myRequests","history","telegramPersonal","pwa","transfusionJournal","reactions","sign"]
    else:
...
def api_stock_summary_v6415():
    """V6.4.33: підсумок складу для деталей компонентів. Працює однаково в SQLite/PostgreSQL."""
    try:
        delta=_stock_delta_sql()
        items=[]
        try:
...
def api_trash_v615_final():
    """V6.4.33: кошик показує реальні записи з таблиці trash + старі записи зі status deleted/trash.
...
def api_component_stock_v6415():
    """V6.4.33: єдине джерело для блоку «Компоненти на складі».
...
def api_component_stock_writeoff_v6422():
    ...
    pack_no=(d.get('pack_no') or (stock_rec or {}).get('pack_no') or (unit or {}).get('pack_no') or '').strip()
    series=(d.get('series') or (stock_rec or {}).get('series') or (unit or {}).get('series') or '').strip()
    # V6.4.33: для списання через розділ «Компоненти/Склад» серія обов’язкова,
...
def api_version_v618():
    return jsonify(ok=True, version="V6.4.33", title=APP_TITLE)
...
# ================= V6.4.33 PERIODIC MOZ BLOOD COMPONENT REPORTS + STOCK FULL COMPONENTS =================
...
def moz_build_report_data(start, end):
    ...
    issue_rows=[]
    writeoff_rows=[]
    counted_request_writeoffs=set()
    counted_stock_writeoff_codes=set()
    try:
    ...
    for x in stock:
        ...
        group=x.get('donor_group') or x.get('group') or ''
        rh=x.get('donor_rh') or x.get('rh') or ''
        amount=_moz_amount(x.get('amount'))
        created=x.get('created_at') or ''
        code=x.get('qr_code') or x.get('pack_no') or x.get('series') or ''
        if amount<=0 or not kind:
            continue
        if _is_before(created, start):
            if kind=='in': _moz_add_with_fallback(summary, component, group, rh, 'opening', amount, code)
            elif kind in ('issue','writeoff'): _moz_add_with_fallback(summary, component, group, rh, 'opening', -amount, code)
        if _in_period(created, start, end):
            if kind=='in':
                _moz_add_with_fallback(summary, component, group, rh, 'received', amount, code)
            elif kind=='issue':
                req=_moz_request_for_stock(x)
                if req and (req.get('status') or '').strip()=='Списано':
                    reason=_txt(req.get('writeoff_reason')).lower()
                    metric='expired' if ('термін' in reason or 'строк' in reason or 'придат' in reason or 'expire' in reason or 'закін' in reason) else 'other_writeoff'
                    _moz_add_with_fallback(summary, component or req.get('component'), group or req.get('donor_group'), rh or req.get('donor_rh'), metric, amount, code)
                    counted_request_writeoffs.add(int(req.get('id') or 0))
                    writeoff_rows.append({'Дата списання': req.get('writeoff_at') or created, 'Пацієнт': req.get('patient_name',''), 'Відділення': req.get('department',''), 'Компонент': component or req.get('component',''), 'Група': moz_norm_group(group or req.get('donor_group','')), 'Rh': moz_norm_rh(rh or req.get('donor_rh','')), 'Кількість': amount, '№ пакета': x.get('pack_no',''), 'Серія': x.get('series',''), 'Термін': x.get('expiry',''), 'Причина': req.get('writeoff_reason','') or x.get('note','')})
                else:
                    _moz_add_with_fallback(summary, component, group, rh, 'used', amount, code)
                    issue_rows.append({'Дата видачі': created, 'Пацієнт': x.get('patient_name',''), 'Відділення': (req or {}).get('department','') if req else '', 'Компонент': component, 'Група': moz_norm_group(group), 'Rh': moz_norm_rh(rh), 'Кількість': amount, '№ пакета': x.get('pack_no',''), 'Серія': x.get('series',''), 'Термін': x.get('expiry',''), 'Примітка': x.get('note','')})
            elif kind=='writeoff':
                note=_txt(x.get('note')).lower()
                metric='expired' if ('термін' in note or 'строк' in note or 'придат' in note or 'expire' in note or 'закін' in note) else 'other_writeoff'
                _moz_add_with_fallback(summary, component, group, rh, metric, amount, code)
                if code: counted_stock_writeoff_codes.add(code)
                writeoff_rows.append({'Дата списання': created, 'Пацієнт': x.get('patient_name',''), 'Відділення': '', 'Компонент': component, 'Група': moz_norm_group(group), 'Rh': moz_norm_rh(rh), 'Кількість': amount, '№ пакета': x.get('pack_no',''), 'Серія': x.get('series',''), 'Термін': x.get('expiry',''), 'Причина': x.get('note','')})

    # Вимоги зі статусом «Списано», якщо окремого руху складу не було.
    try:
        reqs=rows("SELECT * FROM requests WHERE status='Списано' AND writeoff_at IS NOT NULL ORDER BY id DESC")
        for r in reqs:
            rid=int(r.get('id') or 0)
            if rid in counted_request_writeoffs: continue
            dt=r.get('writeoff_at') or ''
            if not _in_period(dt,start,end): continue
            code=r.get('pack_no') or r.get('series') or ''
            if code and code in counted_stock_writeoff_codes: continue
            amount=_moz_amount(r.get('amount') or 1)
            reason=_txt(r.get('writeoff_reason')).lower()
            metric='expired' if ('термін' in reason or 'строк' in reason or 'придат' in reason or 'expire' in reason or 'закін' in reason) else 'other_writeoff'
            _moz_add_with_fallback(summary, r.get('component',''), r.get('donor_group') or r.get('patient_group'), r.get('donor_rh') or r.get('patient_rh'), metric, amount, code)
            writeoff_rows.append({'Дата списання': dt, 'Пацієнт': r.get('patient_name',''), 'Відділення': r.get('department',''), 'Компонент': r.get('component',''), 'Група': moz_norm_group(r.get('donor_group') or r.get('patient_group')), 'Rh': moz_norm_rh(r.get('donor_rh') or r.get('patient_rh')), 'Кількість': amount, '№ пакета': r.get('pack_no',''), 'Серія': r.get('series',''), 'Термін': r.get('expiry',''), 'Причина': r.get('writeoff_reason','')})
    except Exception:
        db_rollback_safe()

    # Старі component_writeoffs.
    try:
        wrows=rows('SELECT * FROM component_writeoffs ORDER BY created_at ASC, id ASC')
        for w in wrows:
            dt=w.get('created_at') or ''
            if not _in_period(dt, start, end): continue
            code=w.get('package_code') or ''
            if code and code in counted_stock_writeoff_codes: continue
            component=w.get('component') or ''
            amount=_moz_amount(w.get('amount'))
            if amount<=0: continue
            c2,g2,rh2=_moz_identity_from_code(code)
            component=component or c2
            note=_txt((w.get('reason') or '') + ' ' + (w.get('notes') or '')).lower()
            metric='expired' if ('термін' in note or 'строк' in note or 'придат' in note or 'expire' in note or 'закін' in note) else 'other_writeoff'
            _moz_add_with_fallback(summary, component, g2, rh2, metric, amount, code)
            writeoff_rows.append({'Дата списання': dt, 'Пацієнт': '', 'Відділення': '', 'Компонент': component, 'Група': moz_norm_group(g2), 'Rh': moz_norm_rh(rh2), 'Кількість': amount, '№ пакета': code, 'Серія': code, 'Термін': '', 'Причина': w.get('reason') or w.get('notes') or ''})
    except Exception:
        db_rollback_safe()

    for comp in list(summary.keys()):
        for key, m in summary[comp].items():
            m['closing']=m['opening']+m['received']-m['used']-m['expired']-m['other_writeoff']

    # Деталізація видачі з requests без дублювання.
    try:
        reqs=rows("SELECT id,issued_at,used_at,patient_name,department,component,amount,donor_group,donor_rh,pack_no,series,expiry,status FROM requests WHERE (issued_at IS NOT NULL OR status IN ('Видано','Використано')) AND status<>'Списано' ORDER BY id DESC")
        existing={(r.get('№ пакета'), r.get('Серія'), _date_part(r.get('Дата видачі'))) for r in issue_rows}
        for r in reqs:
            dt=r.get('issued_at') or r.get('used_at') or ''
            key=(r.get('pack_no'), r.get('series'), _date_part(dt))
            if _in_period(dt,start,end) and key not in existing:
                issue_rows.append({'Дата видачі': dt, 'Пацієнт': r.get('patient_name',''), 'Відділення': r.get('department',''), 'Компонент': r.get('component',''), 'Група': moz_norm_group(r.get('donor_group','')), 'Rh': moz_norm_rh(r.get('donor_rh','')), 'Кількість': r.get('amount') or 0, '№ пакета': r.get('pack_no',''), 'Серія': r.get('series',''), 'Термін': r.get('expiry',''), 'Примітка': f"Вимога №{r.get('id')} · {r.get('status','')}"})
    except Exception:
        db_rollback_safe()
    return summary, issue_rows, writeoff_rows
...
def api_reports_moz_period_summary_v643():
    ...
    summary, issues, writeoffs=moz_build_report_data(start,end)
    return jsonify(ok=True, period={'type':ptype,'date_from':start.isoformat(),'date_to':end.isoformat(),'label':label,'issued_at':now(), 'units':'дози/од.'}, rows=moz_flat_summary(summary), issues=issues[:200], writeoffs=writeoffs[:200])
...
# ================= END V6.4.33 PERIODIC MOZ BLOOD COMPONENT REPORTS + STOCK FULL COMPONENTS =================
