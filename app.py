...
def health_payload():
    ...
    return {"ok":ok,"version":"V6.4.16","database":"ok" if ok else "error","database_error":err,"postgres":IS_POSTGRES,"telegram_configured":bool(TELEGRAM_BOT_TOKEN and TELEGRAM_CHAT_ID),"backup_age_hours":age,"auto_backup_enabled":AUTO_BACKUP_ENABLED,
...
def api_external_status():
    return jsonify(ok=True, version="V6.4.16", postgres=IS_POSTGRES, time=now())
...
def api_tx_reset_v594():
    db_rollback_safe()
    return jsonify(ok=True, version="V6.4.16", message="transaction rolled back")


@app.get("/api/ui/feature-map")
@login_required
def api_ui_feature_map():
    return jsonify(ok=True, version="V6.4.16", features={
...
def api_ui_role_config_v613():
    ...
    if role in ("admin","transfusion"):
        ...
        # V6.4.16: doctor має бачити тільки робочі персональні розділи, без складу/моніторингу/системи
        allowed=["home","patients","requests","myRequests","history","telegramPersonal","pwa","transfusionJournal","reactions","sign"]
    elif role=="nurse":
        # V6.4.16: nurse має бачити тільки робочі персональні розділи, без складу/моніторингу/системи
        allowed=["home","patients","requests","myRequests","history","telegramPersonal","pwa","transfusionJournal","reactions","sign"]
    else:
...
def api_stock_summary_v6415():
    """V6.4.16: підсумок складу для деталей компонентів. Працює однаково в SQLite/PostgreSQL."""
    try:
        delta=_stock_delta_sql()
        items=[]
        try:
...
def api_component_stock_v6415():
    """V6.4.16: єдине джерело для блоку «Компоненти на складі».
...
def api_version_v618():
    return jsonify(ok=True, version="V6.4.16", title=APP_TITLE)
...
# ================= V6.4.16 PERIODIC MOZ BLOOD COMPONENT REPORTS + STOCK FULL COMPONENTS =================
...
# ================= END V6.4.16 PERIODIC MOZ BLOOD COMPONENT REPORTS + STOCK FULL COMPONENTS =================
