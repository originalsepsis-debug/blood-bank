...
def health_payload():
    ...
    return {"ok":ok,"version":"V6.1.6","database":"ok" if ok else "error","database_error":err,"postgres":IS_POSTGRES,"telegram_configured":bool(TELEGRAM_BOT_TOKEN and TELEGRAM_CHAT_ID),"backup_age_hours":age,"auto_backup_enabled":AUTO_BACKUP_ENABLED,
...
def api_external_status():
    return jsonify(ok=True, version="V6.1.6", postgres=IS_POSTGRES, time=now())
...
def api_tx_reset_v594():
    db_rollback_safe()
    return jsonify(ok=True, version="V6.1.6", message="transaction rolled back")
...
ROLE_PERMISSIONS = {
    ...
    "nurse": {
        "label": "Медсестра",
        "sections": ["dashboard","requests","stock","barcode","traceability","temperature","telegram","pwa"],
        "can_manage_users": False,
        "can_edit_all": False,
        "can_delete_all": False,
        "can_view_audit": False,
...
def api_ui_role_config_v613():
    ...
    if role in ("admin","transfusion"):
        ...
        allowed=["home","patients","requests","myRequests","history","telegramPersonal","pwaPersonal","transfusionJournal","reactions","barcode"]
    else:
...
@app.get("/api/version")
def api_version_v616():
    return jsonify(ok=True, version="V6.1.6", title=APP_TITLE)
