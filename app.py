...
def health_payload():
    ...
    return {"ok":ok,"version":"V6.4.14","database":"ok" if ok else "error","database_error":err,"postgres":IS_POSTGRES,"telegram_configured":bool(TELEGRAM_BOT_TOKEN and TELEGRAM_CHAT_ID),"backup_age_hours":age,"auto_backup_enabled":AUTO_BACKUP_ENABLED,
...
def api_request_used():
    u=current_user(); d=request.json or {}; rid=int(d.get("id"))
...
def api_request_writeoff():
    u=current_user(); d=request.json or {}; rid=int(d.get("id"))
    if u.get("role") in ("doctor","nurse") and not owns_request(u, rid):
        return jsonify(ok=False,error="Можна списувати тільки свої записи"), 403
    if not d.get("writeoff_date") or not d.get("written_by") or not d.get("writeoff_reason"):
        return jsonify(ok=False,error="Заповніть дату, хто списав і причину")
    req=row("SELECT * FROM requests WHERE id=?", (rid,))
...
def api_external_status():
    return jsonify(ok=True, version="V6.4.14", postgres=IS_POSTGRES, time=now())
...
def api_tx_reset_v594():
    db_rollback_safe()
    return jsonify(ok=True, version="V6.4.14", message="transaction rolled back")


@app.get("/api/ui/feature-map")
@login_required
def api_ui_feature_map():
    return jsonify(ok=True, version="V6.4.14", features={
...
@app.get("/api/health")
def api_health_v60():
    out = {"ok": False, "version": APP_VERSION}
    try:
        v60_db_safe_reset()
        out["db_select"] = row("SELECT 1 AS ok")
        out["migrations"] = v60_safe_migrations()
        out["admins"] = len(rows("SELECT id FROM users WHERE role='admin' AND active=1"))
        out["ok"] = True
    except Exception as e:
        v60_db_safe_reset()
...
def api_ui_role_config_v613():
    ...
    if role in ("admin","transfusion"):
        ...
        # V6.4.14: doctor має бачити тільки робочі персональні розділи, без складу/моніторингу/системи
        allowed=["home","patients","requests","myRequests","history","telegramPersonal","pwa","transfusionJournal","reactions","sign"]
    elif role=="nurse":
        # V6.4.14: nurse має бачити тільки робочі персональні розділи, без складу/моніторингу/системи
        allowed=["home","patients","requests","myRequests","history","telegramPersonal","pwa","transfusionJournal","reactions","sign"]
    else:
...
@role_required("admin","transfusion")
def api_component_stock_v623():
    """Compatibility endpoint for component stock cards. Uses real net stock, not record count."""
    try:
        delta = _stock_delta_sql()
        items = rows(f"""SELECT component, donor_group, donor_rh, donor_group as 'group', donor_rh as rh,
                       ...
                       HAVING COALESCE(SUM({delta}),0) <> 0
                       ORDER BY component, donor_group, donor_rh""")
        # Fallback for new unit model if there are no classic stock rows yet.
        if not items:
            items = rows("""SELECT component, donor_group, donor_rh, donor_group as 'group', donor_rh as rh,
                                  COALESCE(SUM(amount),0) AS qty, COALESCE(SUM(amount),0) AS amount, MIN(NULLIF(expiry,'')) AS nearest_expiry
                           FROM blood_units
...
def api_version_v618():
    return jsonify(ok=True, version="V6.4.14", title=APP_TITLE)
...
# ================= V6.4.14 PERIODIC MOZ BLOOD COMPONENT REPORTS + STOCK FULL COMPONENTS =================
...
# ================= END V6.4.14 PERIODIC MOZ BLOOD COMPONENT REPORTS + STOCK FULL COMPONENTS =================
