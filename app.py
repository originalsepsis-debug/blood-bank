...
def _parse_backup_dt(value):
    """V6.4.34: parse backup timestamps from DB rows or file mtime fallback."""
    if not value:
        return None
...
def _latest_backup_info():
    """V6.4.34: show last backup from DB; if record is missing, detect newest ZIP/DB backup file in BACKUP_DIR."""
...
def health_payload():
    ...
    return {
        "ok": ok,
        "version": APP_VERSION if 'APP_VERSION' in globals() else "V6.4.34",
...
def forbid_json():
    return jsonify(ok=False, error="Недостатньо прав"), 403
# ================= END V6.4.34 PERMISSION MATRIX =================

@app.route("/")
def index():
    # V6.4.34 FAST LOGIN: do not run schema migrations on every page open.
...
def login():
    ...
    db_rollback_safe()
    # V6.4.34 FAST LOGIN: migrations are not executed during password submit.
...
def api_request_writeoff():
    # V6.4.34: списання дозволене після погодження трансфузіологом.
...
def delete_trash_record_permanent():
    """V6.4.34: остаточно стерти запис із кошика.
...
def api_patient_history():
    # V6.4.34: doctor/nurse бачать історію тільки власних вимог, включно зі списаними/відмовленими/використаними.
...
def api_external_status():
    return jsonify(ok=True, version="V6.4.34", postgres=IS_POSTGRES, time=now())
...
def _traceability_rows_for_code(code):
    """V6.4.34: простежуваність має працювати не тільки з package_traceability.
...
def api_writeoff_component():
    """V6.4.34: списання через окремий розділ має реально зменшувати склад.
...
def api_public_health_v593():
    # V6.4.34: легка публічна перевірка без міграцій і без розкриття службових даних.
    out={"ok":False,"version":"V6.4.34"}
    try:
        row("SELECT 1 AS ok")
        out["ok"]=True
...
def api_tx_reset_v594():
    db_rollback_safe()
    return jsonify(ok=True, version="V6.4.34", message="transaction rolled back")


@app.get("/api/ui/feature-map")
@login_required
def api_ui_feature_map():
    return jsonify(ok=True, version="V6.4.34", features={
...
@app.get("/api/health")
def api_health_v60():
    # V6.4.34: lightweight real home status. No migrations on every health call.
    out = health_payload()
    try:
        out["db_select"] = row("SELECT 1 AS ok")
    except Exception as e:
...
def api_ui_role_config_v613():
    ...
    if role in ("admin","transfusion"):
        ...
        # V6.4.34: doctor має бачити тільки робочі персональні розділи, без складу/моніторингу/системи
        allowed=["home","patients","requests","myRequests","history","telegramPersonal","pwa","transfusionJournal","reactions","sign"]
    elif role=="nurse":
        # V6.4.34: nurse має бачити тільки робочі персональні розділи, без складу/моніторингу/системи
        allowed=["home","patients","requests","myRequests","history","telegramPersonal","pwa","transfusionJournal","reactions","sign"]
    else:
...
def api_stock_summary_v6415():
    """V6.4.34: підсумок складу для деталей компонентів. Працює однаково в SQLite/PostgreSQL."""
    try:
        delta=_stock_delta_sql()
        items=[]
        try:
...
def api_trash_v615_final():
    """V6.4.34: кошик показує реальні записи з таблиці trash + старі записи зі status deleted/trash.
...
def api_component_stock_v6415():
    """V6.4.34: єдине джерело для блоку «Компоненти на складі».
...
def api_component_stock_writeoff_v6422():
    ...
    pack_no=(d.get('pack_no') or (stock_rec or {}).get('pack_no') or (unit or {}).get('pack_no') or '').strip()
    series=(d.get('series') or (stock_rec or {}).get('series') or (unit or {}).get('series') or '').strip()
    # V6.4.34: для списання через розділ «Компоненти/Склад» серія обов’язкова,
...
def api_version_v618():
    return jsonify(ok=True, version="V6.4.34", title=APP_TITLE)
...
# ================= V6.4.34 PERIODIC MOZ BLOOD COMPONENT REPORTS + STOCK FULL COMPONENTS =================
...
def moz_build_report_data(start, end):
    """V6.4.34: audited report builder.
    Source of truth for movement totals is stock_entries only.
    Previous versions also mixed requests/component_writeoffs fallback records, which could double-count
    or create impossible values such as platelets received/written off when there was no real stock movement.
    """
    ...
    def add_row(metric, x, amount, kind):
    ...
    for x in stock:
        ...
        if _is_before(created, start):
            if kind=='in':
                add_row('opening', x, amount, kind)
            elif kind in ('issue','writeoff'):
                add_row('opening', x, -amount, kind)
        if _in_period(created, start, end):
            if kind=='in':
                add_row('received', x, amount, kind)
            elif kind=='issue':
                add_row('used', x, amount, kind)
                req=_moz_request_for_stock(x)
                issue_rows.append({'Дата видачі': created, 'Пацієнт': (req or {}).get('patient_name','') if req else x.get('patient_name',''), 'Відділення': (req or {}).get('department','') if req else '', 'Компонент': component, 'Група': moz_norm_group(group), 'Rh': moz_norm_rh(rh), 'Кількість': amount, '№ пакета': x.get('pack_no',''), 'Серія': x.get('series',''), 'Термін': x.get('expiry',''), 'Примітка': x.get('note','')})
            elif kind=='writeoff':
                ...
                add_row(metric, x, amount, kind)
                req=_moz_request_for_stock(x)
                writeoff_rows.append({'Дата списання': created, 'Пацієнт': (req or {}).get('patient_name','') if req else x.get('patient_name',''), 'Відділення': (req or {}).get('department','') if req else '', 'Компонент': component, 'Група': moz_norm_group(group), 'Rh': moz_norm_rh(rh), 'Кількість': amount, '№ пакета': x.get('pack_no',''), 'Серія': x.get('series',''), 'Термін': x.get('expiry',''), 'Причина': x.get('note','')})
    ...
    for comp in list(summary.keys()):
        for key, m in summary[comp].items():
            ...
            m['closing_raw']=raw
            if raw < 0:
                audit_warnings.append({'component':comp, 'group':key[0], 'rh':key[1], 'message':f'Негативний залишок {raw:g}. Перевірте старі записи надходження/видачі/списання.'})
                # Official preview should not display negative stock as a valid balance.
                m['closing']=0.0
            else:
                m['closing']=raw
...
# ================= END V6.4.34 PERIODIC MOZ BLOOD COMPONENT REPORTS + STOCK FULL COMPONENTS =================
